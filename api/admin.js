...
function classifyInboundMessage(message, subject = "") {
  ...
  if (hasAny(["date", "availability", "available", "book", "booking", "cancel", "reschedule", "payment link", "stripe", "price", "pricing", "quote", "invoice"])) return { classification: hasAny(["price", "pricing", "quote", "payment link", "stripe", "invoice"]) ? "wants pricing" : "booking request", assigned_agent: "theo", status: "pending_approval", action_taken: "Routed to Theo approval queue", approval_required: true };
...
async function saveInboundMessage(supabase, payload) {
  ...
  const { data, error } = await supabase
    ...
    .single();
  if (error) throw error;
  const metadata = { inbound_message_id: data.id, classification: data.classification, source: data.source, from_email: data.from_email };
  ...
  if (route.assigned_agent === "theo") {
    await saveReplyIntake(supabase, {
      reply: {
