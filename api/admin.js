...
async function getTrainingCompliance(supabase) {
  ...
  const [organisationsResult, membersResult, recordsResult, evidenceResult] = await Promise.all([
...
async function getTrainingEvidenceUrl(supabase, payload) {
  ...
  if (fetchError) throw fetchError;
  if (!evidence) throw new Error("Evidence file was not found.");
  const { data, error } = await supabase.storage.from(evidenceBucket).createSignedUrl(evidence.file_path, 300, {
