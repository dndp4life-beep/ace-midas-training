...
function classifyInboundMessage(message, subject = "") {
  ...
  const hasAny = (words) => words.some((word) => text.includes(word));
  if (hasAny(["unsubscribe", "do not contact", "remove me", "opt out"])) return { classification: "unsubscribe/do not contact", assigned_agent: "ellis", status: "filtered", action_taken: "Do not contact flag identified", approval_required: false };
  if (hasAny(["seo", "backlink", "crypto", "loan", "web design", "marketing agency", "guest post"])) return { classification: "spam/B2B irrelevant", assigned_agent: "ellis", status: "archived", action_taken: "Filtered as low-value B2B/spam", approval_required: false };
  const theoRoute = classifyTheoBookingIntent(text);
  if (theoRoute) return theoRoute;
  if (hasAny(["certificate", "evidence", "proof of training"])) return { classification: "certificate request", assigned_agent: "ava", status: "routed", action_taken: "Routed to Ava for compliance/certificate review", approval_required: false };
  if (hasAny(["refresher", "renew", "expired", "expiry", "training due"])) return { classification: "refresher enquiry", assigned_agent: "mia", status: "routed", action_taken: "Routed to Mia for safe refresher follow-up", approval_required: false };
  if (hasAny(["content", "social", "post", "case study", "news"])) return { classification: "new enquiry", assigned_agent: "nia", status: "routed", action_taken: "Routed to Nia for content opportunity review", approval_required: false };
  if (hasAny(["partnership", "provider", "school", "council", "local authority", "trust"])) return { classification: "new enquiry", assigned_agent: "rory", status: "routed", action_taken: "Routed to Rory for research/partnership review", approval_required: false };
  if (hasAny(["information", "details", "tell me more", "course"])) return { classification: "wants more information", assigned_agent: "mia", status: "routed", action_taken: "Routed to Mia for safe information response", approval_required: false };
...
function classifyTheoBookingIntent(text) {
  ...
  if (paymentLinkRequest) return { classification: "payment link request", assigned_agent: "theo", status: "pending_approval", action_taken: "Routed to Theo approval queue because payment links require human approval", approval_required: true, decision_reason: "Payment/custom Stripe links require human approval." };
  if (bookingChangeRequest) return { classification: "booking change request", assigned_agent: "theo", status: "pending_approval", action_taken: "Routed to Theo approval queue because booking changes/cancellations require human approval", approval_required: true, decision_reason: "Booking changes and cancellations require human approval." };
  if (customCommercialRequest) return { classification: "pricing exception request", assigned_agent: "theo", status: "pending_approval", action_taken: "Routed to Theo approval queue because pricing exceptions require human approval", approval_required: true, decision_reason: "Discounts, custom prices and exceptions require human approval." };
  if (commitmentRequest) return { classification: "booking confirmation request", assigned_agent: "theo", status: "pending_approval", action_taken: "Routed to Theo approval queue because the message asks for a booking/date commitment", approval_required: true, decision_reason: "Exact dates, confirmed availability or booking commitments require human approval." };
  if (hasAny(["how do i book", "how do we book", "how to book", "what information do you need", "what info do you need", "what details do you need"])) return { classification: "booking process question", assigned_agent: "theo", status: "auto_reply", action_taken: "Theo can safely explain the booking process and ask for required details", approval_required: false, decision_reason: "General booking process question with no commitment requested." };
  if (hasAny(["group booking", "group bookings", "multiple staff", "several staff", "team booking"])) return { classification: "group booking enquiry", assigned_agent: "theo", status: "auto_reply", action_taken: "Theo can safely explain group booking requirements", approval_required: false, decision_reason: "Group booking explanation is allowed without confirming dates, prices or availability." };
  if (hasAny(["onsite", "on-site", "on site", "come to us", "at our site", "at our depot"])) return { classification: "onsite training enquiry", assigned_agent: "theo", status: "auto_reply", action_taken: "Theo can safely explain onsite training requirements", approval_required: false, decision_reason: "Onsite training explanation is allowed without making a commitment." };
  if (hasAny(["what dates are available", "dates available", "availability", "available dates", "course availability", "are you available"])) return { classification: "availability enquiry", assigned_agent: "theo", status: "auto_reply", action_taken: "Theo can ask for preferred timeframe, location and attendee count without promising availability", approval_required: false, decision_reason: "General availability enquiry can be answered safely without confirming exact dates." };
  if (hasAny(["price", "pricing", "cost", "quote", "how much"])) return { classification: "pricing enquiry", assigned_agent: "theo", status: "auto_reply", action_taken: "Theo can explain pricing depends on course type, attendee numbers and location", approval_required: false, decision_reason: "General pricing structure enquiry can be answered safely without confirming a final price." };
  if (hasAny(["book", "booking", "attendees", "delegates", "drivers", "refresher booking"])) return { classification: "booking information request", assigned_agent: "theo", status: "auto_reply", action_taken: "Theo can gather course type, attendee numbers, location and preferred timeframe", approval_required: false, decision_reason: "Safe booking information gathering does not confirm dates, prices or payment links." };
  return null;
}
...
function calculateTheoEstimate(message) {
  ...
  return {
    ...
    discount_text: saving > 0 ? `Website group discount applied, saving approximately £${saving}.` : "No group discount is triggered at this attendee number.",
...
async function saveInboundMessage(supabase, payload) {
  ...
  const theoTrace = route.assigned_agent === "theo" ? createTheoDecisionTrace(data.id, route, { pricing_estimate_calculated: Boolean(estimate), training_page_referred: !route.approval_required, pricing_estimate: estimate, training_page_url: !route.approval_required ? theoTrainingPageUrl : null }) : null;
  const metadata = { inbound_message_id: data.id, classification: data.classification, source: data.source, from_email: data.from_email, decision_reason: route.decision_reason || null, ...(theoTrace ? { theo_trace: theoTrace } : {}) };
