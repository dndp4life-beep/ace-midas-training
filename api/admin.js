...
async function getTrainingCompliance(supabase) {
  ...
  const [organisationsResult, membersResult, recordsResult, evidenceResult, remindersResult, reminderLogsResult, agentLogsResult, prospectsResult, followUpsResult, repliesResult, inboundResult, contentDraftsResult] = await Promise.all([
...
function normaliseProspectStatus(status) {
  ...
  return ["new", "pending_review", "approved", "rejected", "pending_outreach", "pending_outreach_approval", "contacted", "follow_up_scheduled", "do_not_contact"].includes(value) ? value : "new";
}
...
function scoreProspect(prospect = {}) {
  ...
  return { priority, recommended_service, relevance_reason };
}
...
async function scoreRoryProspect(supabase, payload) {
  ...
  await insertAgentLog(supabase, {
    ...
    summary: `${rory.name} scored ${data.organisation_name} as ${data.priority} priority and recommended ${data.recommended_service}.`,
    status: "scored",
    approval_required: true,
    metadata: { prospect_id: data.id, priority: data.priority, recommended_service: data.recommended_service, outreach_brief }
  });
  const compliance = await getTrainingCompliance(supabase);
  return { success: true, prospect: data, prospects: compliance.prospects, followUps: compliance.followUps, agentLogs: compliance.agentLogs };
}
...
async function sendProspectToMia(supabase, payload) {
  ...
  const prospectUpdate = {
    assigned_to: "Mia",
    status: sendStatus === "sent" ? "contacted" : sendStatus === "failed" ? "pending_outreach" : "pending_outreach_approval",
