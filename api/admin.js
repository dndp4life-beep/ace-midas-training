...
const ellisEmailSelect = "id, owner_id, mailbox_connection_id, external_message_id, source, sender_name, sender_email, subject, received_at, raw_excerpt, summary, category, priority, confidence_score, recommended_action, requires_review, review_status, reasoning_metadata, contact_id, organisation_id, assigned_route, crm_enriched_at, created_at, updated_at";
const ellisTaskSelect = "id, owner_id, related_email_id, contact_id, organisation_id, title, description, due_date, priority, status, assigned_user, created_at, updated_at";
const ellisBriefingSelect = "id, owner_id, briefing_date, summary, metrics, urgent_emails, recommendations, status, created_at, updated_at";
const ellisActivitySelect = "id, owner_id, email_triage_id, task_id, action_type, summary, metadata, created_at";
const mailboxConnectionSelect = "id, owner_id, provider, mailbox_email, display_name, status, scopes, token_expires_at, last_synced_at, metadata, created_at, updated_at";
const crmContactSelect = "id, owner_id, full_name, email_address, phone_number, organisation_id, organisation, job_title, contact_type, tags, notes, first_contact_date, last_contact_date, interaction_count, enquiry_count, booking_count, estimated_value, relationship_score, relationship_category, status, created_at, updated_at";
const crmOrganisationSelect = "id, name, organisation_type, contact_name, contact_email, phone, website, notes, first_contact_date, last_contact_date, interaction_count, enquiries_received, bookings_completed, estimated_lifetime_value, relationship_score, updated_at";
const crmInteractionSelect = "id, owner_id, contact_id, organisation_id, email_triage_id, assigned_user, opportunity_reference, interaction_type, subject, summary, metadata, occurred_at, created_at";
...
const ellisTaskStatuses = ["Open", "In Progress", "Waiting", "Completed"];
...
function normaliseEllisChoice(value, allowed, fallback) {
  const selected = String(value || "").trim();
  return allowed.includes(selected) ? selected : fallback;
}

function analyseEllisEmail(payload) {
  const senderName = String(payload?.sender_name || "").trim();
  const senderEmail = String(payload?.sender_email || "").trim().toLowerCase();
  const subject = String(payload?.subject || "").trim();
  const rawExcerpt = String(payload?.raw_excerpt || payload?.message || "").trim();
  const text = `${senderName} ${senderEmail} ${subject} ${rawExcerpt}`.toLowerCase();
  const matches = [];
  const addMatches = (rule, phrases) => {
    const found = includesAny(text, phrases);
    if (found.length) matches.push({ rule, matched: found });
    return found.length > 0;
  };

  const legal = addMatches("legal_or_compliance", ["legal", "solicitor", "compliance", "data protection", "gdpr", "complaint", "safeguarding", "contract", "policy", "audit"]);
  const invoice = addMatches("invoice_or_payment", ["invoice", "payment", "paid", "overdue", "remittance", "purchase order", "po number", "stripe", "receipt"]);
  const council = addMatches("council_or_local_authority", ["council", "local authority", "borough", ".gov.uk", "commissioning", "procurement"]);
  const school = addMatches("school_or_academy", ["school", "academy", "academy trust", "mat ", "education", ".sch.uk", "sendco", "headteacher"]);
  const booking = addMatches("booking_request", ["book", "booking", "availability", "available", "dates", "schedule", "quote", "attendees", "delegates", "training request"]);
  const customer = addMatches("customer_enquiry", ["enquiry", "training", "pats", "midas", "first aid", "refresher", "certificate", "course", "support"]);
  const supplier = addMatches("supplier", ["supplier", "vendor", "statement", "account manager", "renewal", "service provider"]);
  ...
  if (spam || salesPitch) {
    ...
    priority = "High";
    recommendedAction = booking || customer ? "Draft Reply" : "Review";
    confidenceScore = 90;
  } else if (school) {
    ...
    priority = "High";
    recommendedAction = "Follow-Up Required";
    confidenceScore = 84;
  } else if (followUp) {
    ...
    category = "Marketing";
    priority = "Low";
    recommendedAction = "Mark Marketing";
    confidenceScore = 82;
  }
  ...
  const summary = summarySource.length > 220 ? `${summarySource.slice(0, 217).trim()}...` : summarySource;
  ...
  return {
    ...
    reasoning_metadata: {
      version: "ellis_phase_1_rules_v1",
...
async function insertEllisActivity(supabase, entry) {
  const { error } = await supabase.from("ellis_activity_log").insert({
    owner_id: entry.owner_id || null,
    email_triage_id: entry.email_triage_id || null,
...
function buildEllisMetrics(emails) {
  const list = Array.isArray(emails) ? emails : [];
  return {
    total_emails_processed: list.length,
    high_priority_count: list.filter((email) => ["Critical", "High"].includes(email.priority)).length,
    customer_enquiries: list.filter((email) => ["Customer Enquiry", "Booking Request"].includes(email.category)).length,
    council_emails: list.filter((email) => email.category === "Council / Local Authority").length,
    school_emails: list.filter((email) => email.category === "School / Academy Trust").length,
    follow_ups_required: list.filter((email) => email.recommended_action === "Follow-Up Required" || email.category === "Follow-Up Required").length,
    invoice_payment_emails: list.filter((email) => email.category === "Invoice / Payment").length,
    marketing_emails: list.filter((email) => email.category === "Marketing").length,
    likely_spam_count: list.filter((email) => email.category === "Likely Spam").length
  };
}

function buildEllisRecommendations(emails) {
  return (Array.isArray(emails) ? emails : [])
    .filter((email) => email.requires_review || ["Critical", "High"].includes(email.priority))
    .slice(0, 8)
    .map((email) => {
      if (email.category === "Council / Local Authority") return `Reply to ${email.sender_name || email.sender_email} council enquiry`;
      if (email.category === "Invoice / Payment") return `Review invoice or payment email from ${email.sender_name || email.sender_email}`;
      if (email.category === "Booking Request") return `Follow up booking request from ${email.sender_name || email.sender_email}`;
      if (email.category === "School / Academy Trust") return `Review school or academy email from ${email.sender_name || email.sender_email}`;
      return `${email.recommended_action}: ${email.subject}`;
    });
}

function crmRelationshipCategory(score) {
  if (score >= 80) return "Critical";
  if (score >= 55) return "High Value";
  if (score >= 30) return "Medium Value";
  if (score > 0) return "Low Value";
  return "Unknown";
}

function crmContactType(email) {
  if (email.category === "Council / Local Authority") return "Council";
  if (email.category === "School / Academy Trust") return "School";
  if (email.category === "Supplier" || email.category === "Invoice / Payment") return "Supplier";
  if (["Customer Enquiry", "Booking Request", "Follow-Up Required"].includes(email.category)) return "Customer";
  return "Other";
}

function domainOrganisationName(senderEmail) {
  const domain = String(senderEmail || "").split("@")[1]?.toLowerCase() || "";
  if (!domain || ["gmail.com", "googlemail.com", "hotmail.co.uk", "hotmail.com", "outlook.com", "yahoo.com"].includes(domain)) return "";
  return domain
    .split(".")[0]
    .split(/[-_]/)
    .filter(Boolean)
...
function ellisRouteForEmail(email) {
  if (email.category === "Booking Request") return "Theo";
  if (["Customer Enquiry", "Follow-Up Required"].includes(email.category)) return "Mia";
  if (email.category === "Compliance / Legal") return "Marvin";
  if (["Invoice / Payment", "Council / Local Authority", "School / Academy Trust"].includes(email.category)) return "Marvin";
  return "Ellis";
}
...
async function findOrCreateCrmOrganisation(supabase, email) {
  ...
  if (!domain || ["gmail.com", "googlemail.com", "hotmail.co.uk", "hotmail.com", "outlook.com", "yahoo.com", "ace-midas-training.co.uk"].includes(domain)) return null;
  const { data: matches, error: findError } = await supabase
    .from("organisations")
    .select(crmOrganisationSelect)
    .or(`contact_email.ilike.%@${domain},website.ilike.%${domain}%`)
    .limit(1);
  if (findError) throw findError;
  if (matches?.[0]) return matches[0];
  const name = domainOrganisationName(senderEmail);
  if (!name) return null;
  const { data: namedMatches, error: nameError } = await supabase
    .from("organisations")
    .select(crmOrganisationSelect)
    .ilike("name", name)
    .limit(1);
  if (nameError) throw nameError;
  if (namedMatches?.[0]) return namedMatches[0];
  const now = new Date().toISOString();
  const { data, error } = await supabase.from("organisations").insert({
    name,
    organisation_type: crmContactType(email) === "Council" ? "Local Authority" : crmContactType(email),
    contact_name: email.sender_name || null,
    contact_email: senderEmail,
    first_contact_date: now,
    last_contact_date: now,
    updated_at: now
  }).select(crmOrganisationSelect).single();
  if (error) throw error;
  return data;
}

async function refreshCrmRelationship(supabase, contactId, organisationId) {
  if (!contactId) return;
  const { data: interactions, error } = await supabase
    .from("crm_interactions")
    .select("id, interaction_type, occurred_at")
    .eq("contact_id", contactId);
  if (error) throw error;
  const list = interactions || [];
  const enquiryCount = list.filter((interaction) => ["Email", "Enquiry"].includes(interaction.interaction_type)).length;
  const bookingCount = list.filter((interaction) => interaction.interaction_type === "Booking").length;
  const recent = list.some((interaction) => new Date(interaction.occurred_at).getTime() >= Date.now() - 30 * 86400000);
  const score = Math.min(100, list.length * 4 + enquiryCount * 5 + bookingCount * 18 + (recent ? 10 : 0));
  const category = crmRelationshipCategory(score);
  const now = new Date().toISOString();
  const { error: contactError } = await supabase.from("crm_contacts").update({
    interaction_count: list.length,
  ...
  if (list.length >= 2) {
    const insight = {
      ...
      insight_type: "repeat_contact_activity",
      insight_text: `This contact has ${list.length} recorded interaction(s), including ${enquiryCount} enquiry interaction(s).`,
      priority: list.length >= 5 ? "High" : "Medium",
      status: "Active",
      metadata: { interaction_count: list.length, enquiry_count: enquiryCount, booking_count: bookingCount },
      updated_at: now
    };
    const { data: existingInsight, error: insightFindError } = await supabase
      .from("crm_insights")
      .select("id")
      .eq("contact_id", contactId)
      .eq("insight_type", "repeat_contact_activity")
      .maybeSingle();
    if (insightFindError) throw insightFindError;
    const { error: insightError } = existingInsight
      ? await supabase.from("crm_insights").update(insight).eq("id", existingInsight.id)
      : await supabase.from("crm_insights").insert(insight);
    if (insightError) throw insightError;
  }
  if (organisationId) {
    const { data: organisationInteractions, error: organisationError } = await supabase
      .from("crm_interactions")
      .select("id, interaction_type")
      .eq("organisation_id", organisationId);
    if (organisationError) throw organisationError;
    const organisationList = organisationInteractions || [];
    const organisationScore = Math.min(100, organisationList.length * 5 + (recent ? 10 : 0));
    const { error: updateOrganisationError } = await supabase.from("organisations").update({
      interaction_count: organisationList.length,
      enquiries_received: organisationList.filter((interaction) => ["Email", "Enquiry"].includes(interaction.interaction_type)).length,
      bookings_completed: organisationList.filter((interaction) => interaction.interaction_type === "Booking").length,
      relationship_score: organisationScore,
      last_contact_date: now,
      updated_at: now
    }).eq("id", organisationId);
    if (updateOrganisationError) throw updateOrganisationError;
  }
  await supabase.from("relationship_scores").insert({
    ...
    score,
    category,
    factors: { interactions: list.length, enquiries: enquiryCount, bookings: bookingCount, recent_contact: recent }
  });
}

async function enrichEllisCrmMemory(supabase) {
  const { data: pending, error } = await supabase
    .from("email_triage")
    .select(ellisEmailSelect)
    .is("crm_enriched_at", null)
    .not("category", "in", '("Likely Spam","Marketing","Sales Pitch")')
  ...
  for (const email of pending || []) {
    ...
    const organisation = await findOrCreateCrmOrganisation(supabase, email);
    ...
    if (!contact) {
      const { data, error: insertError } = await supabase.from("crm_contacts").insert({
        full_name: email.sender_name || null,
        email_address: senderEmail,
        organisation_id: organisation?.id || null,
        organisation: organisation?.name || null,
        contact_type: crmContactType(email),
        first_contact_date: email.received_at || now,
        last_contact_date: email.received_at || now
      }).select(crmContactSelect).single();
      if (insertError) throw insertError;
      contact = data;
    }
    const route = ellisRouteForEmail(email);
    const { error: interactionError } = await supabase.from("crm_interactions").upsert({
      contact_id: contact.id,
      organisation_id: organisation?.id || contact.organisation_id || null,
      email_triage_id: email.id,
      assigned_user: route,
      interaction_type: ["Booking Request", "Customer Enquiry", "Follow-Up Required"].includes(email.category) ? "Enquiry" : "Email",
      subject: email.subject,
      summary: email.summary,
      metadata: { category: email.category, priority: email.priority, source: email.source },
      occurred_at: email.received_at || now
    }, { onConflict: "email_triage_id" });
    if (interactionError) throw interactionError;
    const organisationId = organisation?.id || contact.organisation_id || null;
    const { error: enrichError } = await supabase.from("email_triage").update({
      contact_id: contact.id,
    ...
    await refreshCrmRelationship(supabase, contact.id, organisationId);
  }
}

function buildEllisCrmMetrics(contacts, organisations, interactions, learningEvents) {
  const contactList = Array.isArray(contacts) ? contacts : [];
  const organisationList = Array.isArray(organisations) ? organisations : [];
  const learningList = Array.isArray(learningEvents) ? learningEvents : [];
  const confirmed = learningList.filter((event) => !event.was_undo).length;
  const overrides = learningList.filter((event) => event.was_override && !event.was_undo).length;
  return {
    contact_count: contactList.length,
    organisation_count: organisationList.length,
    interaction_count: Array.isArray(interactions) ? interactions.length : 0,
    high_value_contacts: contactList.filter((contact) => ["Critical", "High Value"].includes(contact.relationship_category)).length,
    total_learning_events: learningList.length,
    confirmed_patterns: confirmed,
    routing_accuracy: confirmed ? Math.round(((confirmed - overrides) / confirmed) * 100) : 0,
    auto_routing_stage: confirmed >= 150 ? 4 : confirmed >= 76 ? 3 : confirmed >= 26 ? 2 : 1
  };
}
...
async function getEllisOperations(supabase) {
  ...
  const [emailsResult, tasksResult, briefingsResult, activityResult, connectionsResult, contactsResult, organisationsResult, interactionsResult, insightsResult, learningResult, historyResult, syncHistoryResult, domainIntelResult, delegationsResult, workQueueResult, alertsResult, alertSettingsResult] = await Promise.all([
    ...
    supabase.from("ellis_activity_log").select(ellisActivitySelect).order("created_at", { ascending: false }).limit(200),
    supabase.from("mailbox_connections").select(mailboxConnectionSelect).order("created_at", { ascending: false }).limit(25),
    supabase.from("crm_contacts").select(crmContactSelect).order("relationship_score", { ascending: false }).limit(200),
    supabase.from("organisations").select(crmOrganisationSelect).order("relationship_score", { ascending: false }).limit(200),
  ...
  return {
    ...
    crm: {
      ...
      action_history: historyResult.data || [],
...
async function saveEllisEmail(supabase, payload) {
  const analysed = analyseEllisEmail(payload?.email || payload);
  if (!analysed.sender_email || !analysed.subject) throw new Error("Sender email and subject are required.");
  const row = {
    ...analysed,
    owner_id: payload?.owner_id || null,
    mailbox_connection_id: payload?.mailbox_connection_id || null,
    external_message_id: String(payload?.external_message_id || "").trim() || null,
    source: String(payload?.source || "manual").trim() || "manual"
  };
  const { data, error } = await supabase.from("email_triage").insert(row).select(ellisEmailSelect).single();
  if (error) throw error;
  await insertEllisActivity(supabase, {
    owner_id: row.owner_id,
    email_triage_id: data.id,
    action_type: "email_triaged",
    summary: `Ellis triaged "${data.subject}" as ${data.category}.`,
    metadata: { priority: data.priority, confidence_score: data.confidence_score, recommended_action: data.recommended_action }
  });
  return getEllisOperations(supabase);
}

async function updateEllisEmail(supabase, payload) {
  const id = payload?.id;
  if (!id) throw new Error("Email triage id is required.");
  const { data: previous, error: previousError } = await supabase.from("email_triage").select(ellisEmailSelect).eq("id", id).single();
  if (previousError) throw previousError;
  const updates = { updated_at: new Date().toISOString() };
  if (payload.category) updates.category = normaliseEllisChoice(payload.category, ellisCategories, "Review Later");
  if (payload.priority) updates.priority = normaliseEllisChoice(payload.priority, ellisPriorities, "Medium");
  if (payload.recommended_action) updates.recommended_action = normaliseEllisChoice(payload.recommended_action, ellisActions, "Review");
  if (payload.review_status) updates.review_status = normaliseEllisChoice(payload.review_status, ellisReviewStatuses, "Pending");
  if (payload.assigned_route) updates.assigned_route = normaliseEllisChoice(payload.assigned_route, ["Ellis", "Mia", "Theo", "Rory", "Ava", "Marvin"], "Ellis");
  if (Object.prototype.hasOwnProperty.call(payload, "requires_review")) updates.requires_review = Boolean(payload.requires_review);
  const { data, error } = await supabase.from("email_triage").update(updates).eq("id", id).select(ellisEmailSelect).single();
  if (error) throw error;
  const changedFields = Object.keys(updates).filter((key) => key !== "updated_at" && previous[key] !== data[key]);
  if (changedFields.length) {
    const wasOverride = changedFields.some((key) => ["category", "priority", "assigned_route"].includes(key));
    const { error: historyError } = await supabase.from("ellis_action_history").insert({
      email_id: data.id,
      action_taken: changedFields.join(", "),
      previous_state: previous,
      new_state: data
    });
    if (historyError) throw historyError;
    const { error: learningError } = await supabase.from("ellis_learning_events").insert({
      email_id: data.id,
      contact_id: data.contact_id || null,
      organisation_id: data.organisation_id || null,
      original_category: previous.category,
      original_priority: previous.priority,
      original_agent_suggestion: previous.assigned_route || "Ellis",
      user_selected_agent: data.assigned_route || previous.assigned_route || "Ellis",
      action_type: "email_review_decision",
      was_override: wasOverride,
      confidence_before: previous.confidence_score,
      confidence_after: data.confidence_score,
  ...
  await insertEllisActivity(supabase, {
    ...
    email_triage_id: data.id,
    action_type: "email_review_updated",
    summary: `Ellis review state updated for "${data.subject}".`,
    metadata: updates
  });
  return getEllisOperations(supabase);
}

async function undoEllisEmailAction(supabase, payload) {
  const emailId = payload?.id;
  if (!emailId) throw new Error("Email triage id is required.");
  const { data: history, error: historyError } = await supabase
    .from("ellis_action_history")
    .select(ellisActionHistorySelect)
    .eq("email_id", emailId)
    .eq("is_undone", false)
    ...
    .maybeSingle();
  if (historyError) throw historyError;
  if (!history) throw new Error("No Ellis action is available to undo.");
  const previous = history.previous_state || {};
  const restore = {
    category: previous.category || "Review Later",
    priority: previous.priority || "Medium",
    recommended_action: previous.recommended_action || "Review",
    requires_review: previous.requires_review !== false,
    review_status: previous.review_status || "Pending",
    assigned_route: previous.assigned_route || "Ellis",
    updated_at: new Date().toISOString()
  };
  const { data, error } = await supabase.from("email_triage").update(restore).eq("id", emailId).select(ellisEmailSelect).single();
  if (error) throw error;
  const { error: markError } = await supabase.from("ellis_action_history").update({ is_undone: true, undone_at: new Date().toISOString() }).eq("id", history.id);
  if (markError) throw markError;
  const { error: learningError } = await supabase.from("ellis_learning_events").insert({
    email_id: data.id,
    contact_id: data.contact_id || null,
    organisation_id: data.organisation_id || null,
    original_category: history.new_state?.category || null,
    original_priority: history.new_state?.priority || null,
    original_agent_suggestion: history.new_state?.assigned_route || null,
    user_selected_agent: restore.assigned_route,
    action_type: "undo_email_action",
    was_override: false,
    was_undo: true,
    confidence_before: data.confidence_score,
    confidence_after: data.confidence_score,
    metadata: { restored_action_history_id: history.id }
  });
  if (learningError) throw learningError;
  await insertEllisActivity(supabase, {
    email_triage_id: data.id,
    action_type: "email_action_undone",
    summary: `Previous Ellis action restored for "${data.subject}".`,
    metadata: { restored_action_history_id: history.id }
  });
  return getEllisOperations(supabase);
}

async function createEllisTask(supabase, payload) {
  const task = payload?.task || payload || {};
  const row = {
    owner_id: task.owner_id || null,
    related_email_id: task.related_email_id || null,
    contact_id: task.contact_id || null,
    organisation_id: task.organisation_id || null,
    title: String(task.title || "").trim(),
    description: String(task.description || "").trim() || null,
    due_date: task.due_date || null,
    priority: normaliseEllisChoice(task.priority, ellisPriorities, "Medium"),
    status: normaliseEllisChoice(task.status, ellisTaskStatuses, "Open"),
    assigned_user: String(task.assigned_user || "Admin").trim() || "Admin"
  };
  if (!row.title) throw new Error("Task title is required.");
  const { data, error } = await supabase.from("ellis_tasks").insert(row).select(ellisTaskSelect).single();
  if (error) throw error;
  if (row.related_email_id) {
    await supabase.from("email_triage").update({ review_status: "Follow-Up Created", recommended_action: "Follow-Up Required", requires_review: true, updated_at: new Date().toISOString() }).eq("id", row.related_email_id);
  }
  await insertEllisActivity(supabase, {
    owner_id: row.owner_id,
    email_triage_id: row.related_email_id,
    task_id: data.id,
    action_type: "follow_up_task_created",
    summary: `Follow-up task created: ${data.title}.`,
    metadata: { due_date: data.due_date, priority: data.priority, assigned_user: data.assigned_user }
  });
  return getEllisOperations(supabase);
}

async function updateEllisTask(supabase, payload) {
  const id = payload?.id;
  if (!id) throw new Error("Ellis task id is required.");
  const updates = { updated_at: new Date().toISOString() };
  if (payload.status) updates.status = normaliseEllisChoice(payload.status, ellisTaskStatuses, "Open");
  if (payload.priority) updates.priority = normaliseEllisChoice(payload.priority, ellisPriorities, "Medium");
  if (Object.prototype.hasOwnProperty.call(payload, "due_date")) updates.due_date = payload.due_date || null;
  if (Object.prototype.hasOwnProperty.call(payload, "assigned_user")) updates.assigned_user = String(payload.assigned_user || "").trim() || "Admin";
  const { data, error } = await supabase.from("ellis_tasks").update(updates).eq("id", id).select(ellisTaskSelect).single();
  if (error) throw error;
  await insertEllisActivity(supabase, {
    owner_id: data.owner_id,
    email_triage_id: data.related_email_id,
    task_id: data.id,
    action_type: "task_updated",
    summary: `Task updated: ${data.title}.`,
    metadata: updates
  });
  return getEllisOperations(supabase);
}
...
async function generateEllisBriefing(supabase) {
  ...
  const { data, error: insertError } = await supabase.from("ellis_daily_briefings").insert({
    ...
    recommendations,
    status: "Generated"
  }).select(ellisBriefingSelect).single();
  if (insertError) throw insertError;
  await insertEllisActivity(supabase, {
    action_type: "daily_briefing_generated",
    summary: "Ellis generated the daily executive inbox briefing.",
    metadata: { briefing_id: data.id, metrics }
  });
  return getEllisOperations(supabase);
}
...
async function syncEllisInbox() {
  ...
  if (!response.ok) {
    console.error("Ellis inbox sync function error:", { status: response.status, result });
    throw new Error(result.error || "Ellis inbox sync could not be started.");
  }
...
function calculateTheoEstimate(message) {
  ...
  return {
    ...
    discount_text: saving > 0 ? `${discountRule.label} applied, saving approximately £${saving}.` : "No configured group discount is triggered at this attendee number.",
...
function uniqueMiaValues(values) {
  return [...new Set((values || []).map((value) => cleanMiaText(value)).filter(Boolean))];
}

function loadMiaSeed() {
  const filePath = path.join(process.cwd(), "src", "data", "miaKnowledgeBaseSeed.json");
  return JSON.parse(fs.readFileSync(filePath, "utf8"));
}

function makeMiaSeedEntry({ category, title, question, approvedAnswer, keywords = [], source = "Mia seed knowledge base", status = "approved", lastUpdated, priority = 50, confidenceThreshold = 35 }) {
  return {
    category: cleanMiaText(category || "General Enquiry"),
    title: cleanMiaText(title || question || category || "Knowledge base entry"),
    question: cleanMiaText(question || ""),
    approved_answer: cleanMiaText(approvedAnswer || ""),
    keywords: uniqueMiaValues(keywords),
    source: cleanMiaText(source || "Mia seed knowledge base"),
    status: cleanMiaText(status || "approved"),
    last_updated: cleanMiaText(lastUpdated || ""),
    priority,
    confidence_threshold: confidenceThreshold
  };
}

function normaliseMiaSeedEntries(seed) {
  const entries = [];
  const lastUpdated = seed?.last_updated || "2026-05-20";
  const serviceKeywords = ["PATS", "MiDAS", "First Aid", "CTFA", "FAW", "EFAW", "Buccal Midazolam", "Refresher Training", "Compliance Tracking Support", "onsite", "group discounts"];
  for (const faq of Array.isArray(seed?.faqs) ? seed.faqs : []) {
    entries.push(makeMiaSeedEntry({
      category: faq.category,
      title: faq.question,
      question: faq.question,
      approvedAnswer: faq.approved_answer,
      keywords: [faq.category, faq.question, ...serviceKeywords],
      source: "Approved FAQ",
      status: faq.status || "approved",
      lastUpdated,
      priority: 90
    }));
  }
  for (const rule of Array.isArray(seed?.delivery_and_pricing_rules) ? seed.delivery_and_pricing_rules : []) {
    entries.push(makeMiaSeedEntry({
      category: rule.rule === "Pricing" ? "Pricing" : rule.rule,
      title: rule.rule,
      question: rule.rule,
      approvedAnswer: rule.approved_wording,
      keywords: [rule.rule, "delivery", "pricing", "group", "onsite", "train-the-trainer"],
      source: "Approved delivery and pricing rule",
      lastUpdated,
      priority: 85
    }));
  }
  const pats = seed?.cta_guidance?.pats || {};
  if (pats.launch) entries.push(makeMiaSeedEntry({ category: "New PATS Scheme", title: "New PATS launch", question: "When did the new PATS course launch?", approvedAnswer: pats.launch, keywords: ["PATS", "new PATS", "launch", "CTA"], source: "CTA guidance", lastUpdated, priority: 80 }));
  if (pats.who_for || pats.structure) entries.push(makeMiaSeedEntry({ category: "PATS", title: "What PATS is for", question: "What is PATS training?", approvedAnswer: `${pats.who_for || ""} ${pats.structure || ""}`, keywords: ["PATS", "passenger assistant", "passengers", "accessible", "wheelchair"], source: "CTA guidance", lastUpdated, priority: 85 }));
  for (const item of Array.isArray(pats.pats_standard) ? pats.pats_standard : []) entries.push(makeMiaSeedEntry({ category: "PATS Standard", title: `PATS Standard - ${item.element}`, question: `How does PATS Standard ${item.element} work?`, approvedAnswer: `${item.element}: ${item.duration || ""}${item.group_size ? `, ${item.group_size}` : ""}. ${item.details || ""}`, keywords: ["PATS Standard", item.element, item.duration, item.group_size, "passenger assistant workshop", "online theory"], source: "CTA guidance", lastUpdated, priority: 80 }));
  for (const item of Array.isArray(pats.pats_accessible) ? pats.pats_accessible : []) entries.push(makeMiaSeedEntry({ category: "PATS Accessible", title: `PATS Accessible - ${item.element}`, question: `How does PATS Accessible ${item.element} work?`, approvedAnswer: `${item.element}: ${item.duration || ""}${item.group_size ? `, ${item.group_size}` : ""}. ${item.details || ""}`, keywords: ["PATS Accessible", item.element, item.duration, item.group_size, "wheelchair", "accessible", "ramp", "lift", "WTORS"], source: "CTA guidance", lastUpdated, priority: 85 }));
  if (pats.planning_note) entries.push(makeMiaSeedEntry({ category: "Group Booking", title: "PATS Accessible large group planning", question: "How many people can you train for wheelchair training?", approvedAnswer: pats.planning_note, keywords: ["PATS Accessible", "wheelchair", "groups of 3", "large group", "50", "passenger assistants"], source: "CTA guidance", lastUpdated, priority: 88 }));
  const midas = seed?.cta_guidance?.midas || {};
  if (midas.who_for) entries.push(makeMiaSeedEntry({ category: "MiDAS", title: "What MiDAS is for", question: "What is MiDAS training?", approvedAnswer: midas.who_for, keywords: ["MiDAS", "minibus", "driver", "driving", "driver training"], source: "CTA guidance", lastUpdated, priority: 85 }));
  for (const item of Array.isArray(midas.midas_standard) ? midas.midas_standard : []) entries.push(makeMiaSeedEntry({ category: "MiDAS Standard", title: `MiDAS Standard - ${item.element}`, question: `How does MiDAS Standard ${item.element} work?`, approvedAnswer: `${item.element}: ${item.duration || ""}. ${item.details || ""}`, keywords: ["MiDAS Standard", item.element, item.duration, "minibus", "driving assessment"], source: "CTA guidance", lastUpdated, priority: 80 }));
  for (const item of Array.isArray(midas.midas_accessible) ? midas.midas_accessible : []) entries.push(makeMiaSeedEntry({ category: "MiDAS Accessible", title: `MiDAS Accessible - ${item.element}`, question: `How does MiDAS Accessible ${item.element} work?`, approvedAnswer: `${item.element}: ${item.duration || ""}. ${item.details || ""}`, keywords: ["MiDAS Accessible", item.element, "wheelchair", "ramps", "lifts", "WTORS", "accessible"], source: "CTA guidance", lastUpdated, priority: 82 }));
  for (const certificate of Array.isArray(seed?.certificate_validity) ? seed.certificate_validity : []) entries.push(makeMiaSeedEntry({ category: "Certificate Validity", title: `${certificate.course} certificate validity`, question: `How long does ${certificate.course} last?`, approvedAnswer: `${certificate.course} certificate validity: ${certificate.validity}.`, keywords: [certificate.course, certificate.validity, "certificate", "validity", "last", "expire"], source: certificate.source || "Approved certificate validity", lastUpdated, priority: 90 }));
  entries.push(makeMiaSeedEntry({ category: "First Aid", title: "First Aid training availability", question: "Do you provide First Aid training?", approvedAnswer: "ACE MiDAS Training provides First Aid training, including Children's Transport First Aid / CTFA, FAW, EFAW and Buccal Midazolam training.", keywords: ["first aid", "ctfa", "faw", "efaw", "buccal midazolam", "workplace first aid"], source: "Approved core services", lastUpdated, priority: 86 }));
  entries.push(makeMiaSeedEntry({ category: "Online Training", title: "Online training guardrail", question: "Do you provide online training?", approvedAnswer: "Some approved course routes include online theory elements, but ACE MiDAS Training provides direct training services and travels to organisations for the relevant in-person training, workshop, practical or assessment elements.", keywords: ["online training", "online theory", "direct training", "onsite"], source: "Approved delivery rule", lastUpdated, priority: 78 }));
  entries.push(makeMiaSeedEntry({ category: "Compliance Tracking Support", title: "Compliance tracking support", question: "Can you tell me about compliance tracking support?", approvedAnswer: "ACE MiDAS Training supports organisations with compliance-related training support, refresher planning and training visibility. Full details of the compliance service and monthly fee can be found on the ACE MiDAS Training compliance page.", keywords: ["compliance app", "compliance tracking", "monthly fee", "service", "software", "dashboard"], source: "Approved business service", lastUpdated, priority: 82 }));
  return entries.filter((entry) => entry.approved_answer && entry.status === "approved");
}

async function getMiaKnowledgeBase(supabase) {
  const [entriesResult, questionsResult] = await Promise.all([
    supabase.from("mia_knowledge_base_entries").select(miaKnowledgeBaseSelect).order("priority", { ascending: false }).order("updated_at", { ascending: false }),
    supabase.from("mia_visitor_questions").select(miaVisitorQuestionSelect).order("created_at", { ascending: false }).limit(200)
  ]);
  ...
  return {
    ...
    counts: {
      ...
      needsReview: (questionsResult.data || []).filter((item) => item.needs_review).length,
      leads: (questionsResult.data || []).filter((item) => item.email || item.phone || item.organisation).length
    }
  };
}

async function syncMiaKnowledgeBase(supabase) {
  const entries = normaliseMiaSeedEntries(loadMiaSeed());
  const { data, error } = await supabase
    .from("mia_knowledge_base_entries")
    .upsert(entries, { onConflict: "category,title" })
    .select(miaKnowledgeBaseSelect);
  if (error) throw error;
  await insertAgentLog(supabase, {
    agent_key: "mia",
    agent_name: "Mia",
    agent_role: "Outreach Coordinator",
    action_type: "mia_knowledge_base_synced",
    action_label: "Knowledge base synced",
    summary: `Mia knowledge base synced from approved seed data with ${data?.length || entries.length} entry/entries.`,
    status: "completed",
    approval_required: false,
    metadata: { entries: data?.length || entries.length }
  }).catch((error) => console.error("Mia KB sync log error:", error));
  return getMiaKnowledgeBase(supabase);
}

async function saveMiaKnowledgeEntry(supabase, payload) {
  const entry = payload?.entry || {};
  if (!cleanMiaText(entry.category) || !cleanMiaText(entry.title) || !cleanMiaText(entry.approved_answer)) throw new Error("Category, title and approved answer are required.");
  const row = {
    category: cleanMiaText(entry.category),
    title: cleanMiaText(entry.title),
    question: cleanMiaText(entry.question),
    approved_answer: cleanMiaText(entry.approved_answer),
    keywords: uniqueMiaValues(Array.isArray(entry.keywords) ? entry.keywords : String(entry.keywords || "").split(",").map((item) => item.trim())),
    source: cleanMiaText(entry.source || "Back Office"),
    status: cleanMiaText(entry.status || "approved"),
    last_updated: entry.last_updated || todayIsoDate(),
    priority: Number(entry.priority || 50),
    confidence_threshold: Number(entry.confidence_threshold || 35),
    updated_at: new Date().toISOString()
  };
  const query = supabase.from("mia_knowledge_base_entries");
  const { error } = entry.id
    ? await query.update(row).eq("id", entry.id)
    : await query.upsert(row, { onConflict: "category,title" });
  if (error) throw error;
  return getMiaKnowledgeBase(supabase);
}

async function updateMiaVisitorQuestion(supabase, payload) {
  const id = payload?.id;
  if (!id) throw new Error("Visitor question id is required.");
  const updates = {
    updated_at: new Date().toISOString()
  };
  for (const key of ["mia_answer", "status", "was_answered", "needs_review"]) {
    if (Object.prototype.hasOwnProperty.call(payload, key)) updates[key] = payload[key];
  }
  const { error } = await supabase.from("mia_visitor_questions").update(updates).eq("id", id);
  if (error) throw error;
  return getMiaKnowledgeBase(supabase);
}

async function createMiaKbEntryFromQuestion(supabase, payload) {
  const questionId = payload?.id;
  if (!questionId) throw new Error("Visitor question id is required.");
  const { data: question, error } = await supabase.from("mia_visitor_questions").select(miaVisitorQuestionSelect).eq("id", questionId).single();
  if (error) throw error;
  const entry = {
    category: cleanMiaText(payload.category || question.course_interest || "General Enquiry"),
    title: cleanMiaText(payload.title || question.visitor_question.slice(0, 120)),
    question: question.visitor_question,
    approved_answer: cleanMiaText(payload.approved_answer || question.mia_answer),
    keywords: uniqueMiaValues([question.course_interest, question.organisation, ...String(question.visitor_question || "").split(" ").filter((word) => word.length > 4).slice(0, 8)]),
    source: "Back Office visitor question",
    status: "approved",
    last_updated: todayIsoDate(),
    priority: 65,
    confidence_threshold: 35
  };
  const { error: upsertError } = await supabase.from("mia_knowledge_base_entries").upsert(entry, { onConflict: "category,title" });
  if (upsertError) throw upsertError;
  await supabase.from("mia_visitor_questions").update({ status: "Answered by Mia", was_answered: true, needs_review: false, updated_at: new Date().toISOString() }).eq("id", questionId);
  return getMiaKnowledgeBase(supabase);
}
...
export default async function handler(req, res) {
  try {
    ...
    const actions = {
      ...
      "get-mia-knowledge-base": getMiaKnowledgeBase,
      "sync-mia-knowledge-base": syncMiaKnowledgeBase,
      "save-mia-knowledge-entry": saveMiaKnowledgeEntry,
      "update-mia-visitor-question": updateMiaVisitorQuestion,
      "create-mia-kb-entry-from-question": createMiaKbEntryFromQuestion,
      "get-ellis-operations": getEllisOperations,
      "save-ellis-email": saveEllisEmail,
      "update-ellis-email": updateEllisEmail,
