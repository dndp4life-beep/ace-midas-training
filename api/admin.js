...
async function runAvaMiaWorkflow(supabase) {
  ...
  for (const record of expiringRecords) {
    ...
    const miaLog = await insertAgentLog(supabase, {
      ...
      status: "completed",
      approval_required: false,
      metadata: { training_record_id: record.id, member_id: member.id, organisation_id: member.organisation_id, course_id: record.course_id, provider_response: providerResponse, agent_title: mia.title, agent_tone: mia.tone, signature: mia.signature }
    });
    logs.push(miaLog);
    sent += 1;
  }
