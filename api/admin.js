...
async function getTrainingCompliance(supabase) {
  ...
  const [organisationsResult, membersResult, recordsResult, evidenceResult, remindersResult, reminderLogsResult, agentLogsResult, prospectsResult, followUpsResult, roryRunsResult, repliesResult, inboundResult, contentDraftsResult] = await Promise.all([
    ...
    supabase.from("prospects").select(prospectSelect).order("created_at", { ascending: false }).limit(100),
    supabase.from("follow_up_tasks").select(followUpTaskSelect).order("scheduled_for", { ascending: true }).limit(100),
    supabase.from("rory_research_runs").select(roryResearchRunSelect).order("started_at", { ascending: false }).limit(50),
...
function scoreProspect(prospect = {}) {
  ...
  const score = Math.min(100, serviceFit + emailScore + Math.min(20, sourceQuality * 4) + (highMatches.length * 8) + (mediumMatches.length * 4));
  ...
  return { priority, recommended_service, relevance_reason, score };
}
...
function normaliseProspectCandidate(candidate = {}, searchTheme = "") {
  ...
  const row = {
    organisation_name: String(candidate.organisation_name || candidate.name || "").trim(),
    ...
    contact_email: String(candidate.contact_email || candidate.email || "").trim(),
    phone: String(candidate.phone || "").trim(),
    decision_maker_name: String(candidate.decision_maker_name || "").trim(),
    likely_training_need: String(candidate.likely_training_need || searchTheme).trim(),
    recommended_service: normaliseRecommendedService(candidate.recommended_service || scored.recommended_service, candidate),
    ...
    source_url: String(candidate.source_url || candidate.website || "").trim(),
    notes: String(candidate.notes || "").trim(),
    relevance_reason: String(candidate.relevance_reason || scored.relevance_reason || "").trim(),
    status: "new",
  ...
  row.outreach_brief = String(candidate.outreach_brief || buildOutreachBrief(row)).trim();
  return row;
}
...
async function pollManusProspectResult(taskId, attempts = 6) {
  ...
  for (let index = 0; index < attempts; index += 1) {
    ...
    if (!response.ok) throw new Error(lastPayload?.error || lastPayload?.message || `Manus task messages returned HTTP ${response.status}`);
...
async function saveRoryProspectCandidates(supabase, { run, candidates, searchTheme, rory }) {
  const { data: existingProspects } = await supabase.from("prospects").select(prospectSelect);
  ...
  for (const candidate of candidates) {
    ...
    if (prospectDuplicateMatch(row, [...existing, ...savedProspects])) {
      duplicatesSkipped += 1;
      await insertAgentLog(supabase, {
        ...
        metadata: { run_id: run.id, organisation_name: row.organisation_name, website: row.website, contact_email: row.contact_email }
      });
      continue;
    }
    const { data: saved, error: saveError } = await supabase.from("prospects").insert(row).select(prospectSelect).single();
    if (saveError) {
      duplicatesSkipped += 1;
      continue;
    }
  ...
  return { prospectsSaved, duplicatesSkipped };
}
...
function buildMiaProspectOutreachEmail(prospect = {}) {
  ...
  const opening = isFirstAid
    ? "Many organisations review First Aid training when staff roles change, certificates approach expiry, or teams grow."
    : isCompliance
      ? "Many organisations need a clearer way to keep training records, refresher dates and compliance evidence organised."
      : "ACE MiDAS Training supports organisations with PATS, MiDAS and refresher training, helping staff remain trained and up to date.";
  const sectorLine = sector && sector !== "other"
    ? `I thought this may be relevant for ${organisation} as a ${sector} with potential staff training or refresher planning needs.`
    : `I thought this may be relevant for ${organisation} if you are reviewing staff training or refresher planning.`;
  const serviceLine = isFirstAid
    ? "We can support First Aid training and refresher planning for staff teams in a practical, professional way."
    : isCompliance
      ? "We can support training compliance tracking, refresher planning and certificate record keeping alongside ACE MiDAS Training services."
      : "We can support PATS, MiDAS, First Aid and refresher training for teams who need to stay current and confident.";
  return `
    <p>Hello,</p>
    <p>${opening}</p>
    <p>${sectorLine}</p>
    <p>${serviceLine}</p>
    <p>If useful, please reply with the type of training you are reviewing and an approximate number of staff, and we can point you in the right direction.</p>
    <p>If this is not relevant, please reply "no thanks" and we will not contact you again.</p>
...
async function scoreRoryProspect(supabase, payload) {
  const id = String(payload?.id || "");
  if (!id) throw new Error("Prospect ID is required.");
  const { data: prospect, error: fetchError } = await supabase.from("prospects").select(prospectSelect).eq("id", id).maybeSingle();
  if (fetchError) throw fetchError;
  ...
  const { data, error } = await supabase
    .from("prospects")
    .update({ ...scored, outreach_brief, updated_at: new Date().toISOString() })
    .eq("id", id)
    .select(prospectSelect)
    .single();
  if (error) throw error;
  ...
  await insertAgentLog(supabase, {
    ...
    status: "scored",
    approval_required: true,
    metadata: { prospect_id: data.id, priority: data.priority, score: data.score, recommended_service: data.recommended_service, outreach_brief }
  });
  const compliance = await getTrainingCompliance(supabase);
  return { success: true, prospect: data, prospects: compliance.prospects, followUps: compliance.followUps, agentLogs: compliance.agentLogs };
}
...
async function sendProspectToMia(supabase, payload) {
  ...
  const followUpOne = addDaysIso(7);
  const followUpTwo = addDaysIso(21);
  ...
  if (canSend) {
    try {
      providerResponse = await sendAgentEmail({
        ...
        subject: "Training support from ACE MiDAS Training",
        html
      });
    } catch (error) {
      sendStatus = "failed";
  ...
  const { data: updatedProspect, error: updateError } = await supabase.from("prospects").update(prospectUpdate).eq("id", prospect.id).select(prospectSelect).single();
  if (updateError) throw updateError;

  const followUpStatus = sendStatus === "sent" ? "pending" : "pending_approval";
  const { error: followUpDeleteError } = await supabase.from("follow_up_tasks").delete().eq("prospect_id", prospect.id).in("task_type", ["follow_up_1", "follow_up_2"]);
  ...
  const { data: followUps, error: followUpError } = await supabase.from("follow_up_tasks").insert([
    { prospect_id: prospect.id, agent_name: "Mia", task_type: "follow_up_1", status: followUpStatus, scheduled_for: followUpOne, notes: "First warm follow-up after Rory prospect handoff." },
    { prospect_id: prospect.id, agent_name: "Mia", task_type: "follow_up_2", status: followUpStatus, scheduled_for: followUpTwo, notes: "Second warm follow-up after Rory prospect handoff." }
  ]).select(followUpTaskSelect);
  ...
  await insertAgentLog(supabase, {
    ...
    summary: `${mia.name} scheduled follow-up tasks for ${prospect.organisation_name}.`,
    status: followUpStatus,
    approval_required: followUpStatus !== "pending",
    metadata: { prospect_id: prospect.id, follow_up_1_scheduled_for: followUpOne, follow_up_2_scheduled_for: followUpTwo }
  });
...
async function runRoryProspectResearch(supabase, payload) {
  ...
  await insertAgentLog(supabase, {
    ...
    status: "running",
    approval_required: false,
    metadata: { run_id: run.id, run_type: runType, search_theme: safeTheme, provider: "manus", endpoint: manusTaskCreateEndpoint }
  });
...
Use publicly available business information only. Prefer generic business emails such as info@, enquiries@, training@, admin@, office@ or hr@. Do not collect private personal data. Return only relevant UK organisations likely to need First Aid, PATS, MiDAS, refresher training or compliance tracking support. Keep results concise and high quality.`,
    ...
    if (!response.ok) throw new Error(createPayload?.error || createPayload?.message || `Manus returned HTTP ${response.status}`);
    ...
    const message = error.message || "Research provider request failed.";
  ...
  try {
    ...
    const message = error.message || "Manus polling failed.";
    const { data: failedRun } = await supabase
      .from("rory_research_runs")
      .update({ status: "failed", errors: message, completed_at: new Date().toISOString() })
      .eq("id", run.id)
    ...
    await insertAgentLog(supabase, {
      ...
      status: "failed",
      approval_required: false,
      metadata: { run_id: run.id, search_theme: safeTheme, provider_task_id: taskId }
    });
    const compliance = await getTrainingCompliance(supabase);
    return { success: false, status: "failed", error: message, run: failedRun || run, roryRuns: compliance.roryRuns, prospects: compliance.prospects, agentLogs: compliance.agentLogs };
  }
  ...
  if (pollResult.status === "waiting_for_result") {
    ...
    await insertAgentLog(supabase, {
      ...
      approval_required: false,
      metadata: { run_id: run.id, search_theme: safeTheme, provider_task_id: taskId, provider_task_url: taskUrl }
    });
    const compliance = await getTrainingCompliance(supabase);
  ...
  const { prospectsSaved, duplicatesSkipped } = await saveRoryProspectCandidates(supabase, { run: { ...run, provider_task_id: taskId }, candidates, searchTheme: safeTheme, rory });
  ...
  await insertAgentLog(supabase, {
    ...
    status: finalStatus,
    approval_required: false,
    metadata: { run_id: run.id, search_theme: safeTheme, provider_task_id: taskId, provider_task_url: taskUrl, prospects_found: candidates.length, prospects_saved: prospectsSaved, duplicates_skipped: duplicatesSkipped }
  });
  const compliance = await getTrainingCompliance(supabase);
  return { success: true, status: finalStatus, run: completedRun || run, roryRuns: compliance.roryRuns, prospects: compliance.prospects, agentLogs: compliance.agentLogs, provider: { name: "manus", task_id: taskId, task_url: taskUrl, live_research_confirmed: true } };
}
...
async function checkRoryResearchRun(supabase, payload) {
  ...
  const candidates = Array.isArray(pollResult.prospects) ? pollResult.prospects : [];
  const { prospectsSaved, duplicatesSkipped } = await saveRoryProspectCandidates(supabase, { run, candidates, searchTheme: run.search_theme || "prospect research", rory });
  const finalStatus = candidates.length ? "completed" : "completed_no_results";
  const { data: completedRun } = await supabase
    .from("rory_research_runs")
  ...
  const compliance = await getTrainingCompliance(supabase);
  return { success: true, status: finalStatus, run: completedRun || run, roryRuns: compliance.roryRuns, prospects: compliance.prospects, agentLogs: compliance.agentLogs };
}
