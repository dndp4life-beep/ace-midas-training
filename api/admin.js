...
async function getTrainingCompliance(supabase) {
  ...
  const [organisationsResult, membersResult, recordsResult, evidenceResult, remindersResult, reminderLogsResult, agentLogsResult, prospectsResult, repliesResult, inboundResult, contentDraftsResult] = await Promise.all([
    ...
    supabase.from("content_drafts").select("id, agent_name, content_type, platform, target_audience, title, content, suggested_visual, call_to_action, hashtags, tone, status, topic, created_at, used_at").order("created_at", { ascending: false }).limit(100)
  ]);
  ...
  return {
    ...
    contentDrafts: contentDraftsResult.error ? [] : contentDraftsResult.data || [],
    counts: {
...
async function generateContentDraft(supabase, payload) {
  ...
  const { data, error } = await supabase
    .from("content_drafts")
    .insert(draft)
    .select("id, agent_name, content_type, platform, target_audience, title, content, suggested_visual, call_to_action, hashtags, tone, status, topic, created_at, used_at")
    .single();
  if (error) throw error;
...
async function deleteContentDraft(supabase, payload) {
  ...
  const { data: existing, error: existingError } = await supabase
    .from("content_drafts")
    .select("id, title, platform, content_type, topic")
    .eq("id", id)
    .maybeSingle();
  if (existingError) throw existingError;
