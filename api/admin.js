...
async function getTrainingCompliance(supabase) {
  ...
  const [organisationsResult, membersResult, recordsResult, evidenceResult, remindersResult, reminderLogsResult, agentLogsResult, prospectsResult, repliesResult] = await Promise.all([
    ...
    supabase.from("reply_intake").select("id, organisation_id, member_id, training_record_id, contact_name, contact_email, message, classification, requested_action, assigned_agent, approval_required, approval_status, notes, created_at, updated_at").order("created_at", { ascending: false }).limit(100)
  ]);
...
async function saveReplyIntake(supabase, payload) {
  ...
  const { data, error } = await supabase
    .from("reply_intake")
    .insert(row)
    .select("id, organisation_id, member_id, training_record_id, contact_name, contact_email, message, classification, requested_action, assigned_agent, approval_required, approval_status, notes, created_at, updated_at")
    .single();
  if (error) throw error;
...
async function updateReplyApproval(supabase, payload) {
  ...
  const { data, error } = await supabase
    .from("reply_intake")
    .update({ approval_status: decision, notes, updated_at: new Date().toISOString() })
    .eq("id", id)
    .select("id, organisation_id, member_id, training_record_id, contact_name, contact_email, message, classification, requested_action, assigned_agent, approval_required, approval_status, notes, created_at, updated_at")
    .single();
  if (error) throw error;
