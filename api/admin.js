...
function buildMiaProspectOutreachEmail(prospect = {}) {
  ...
  const opening = isFirstAid
    ? "First Aid training is often reviewed only when certificates approach expiry, staffing changes or compliance checks bring it into focus. It is easier to have that cover organised before it becomes urgent."
    : isCompliance
      ...
      : isPats
        ? "PATS training is often something organisations only think about when a contract, incident, inspection or renewal date brings it into focus. In reality, it is one of those safeguards that is easier to have in place before it becomes urgent."
        : isMidas
          ? "MiDAS training can be easy to treat as an admin requirement, but for organisations using minibuses it is part of keeping passenger transport safer, more confident and better evidenced."
  ...
  return `
    <div style="font-family:Arial,sans-serif;color:#0f172a;line-height:1.6;max-width:640px">
      ...
      <p>${sectorLine}</p>
      <p>${serviceLine}</p>
      <p>Large group bookings may be eligible for group discounts, which can make it easier for organisations to keep several staff members trained at the same time.</p>
      <p>This is simply a short introduction so you know ACE MiDAS Training is available if your organisation ever needs support with staff training, refresher planning or training compliance.</p>
      <p>You can view the training options here:<br /><a href="${siteUrl}" style="color:#047857;font-weight:700">${siteUrl}</a></p>
...
async function importRoryProspects(supabase, payload) {
  ...
  const { prospectsSaved, duplicatesSkipped } = await saveRoryProspectCandidates(supabase, { run, candidates, searchTheme, rory });
  ...
  const compliance = await getTrainingCompliance(supabase);
  return { success: true, status: finalStatus, run: completedRun || run, roryRuns: compliance.roryRuns, prospects: compliance.prospects, followUps: compliance.followUps, agentLogs: compliance.agentLogs, prospects_saved: prospectsSaved, duplicates_skipped: duplicatesSkipped };
}
