...
function classifyTheoBookingIntent(text) {
  ...
  const groupDiscountQuestion = hasAny(["group discount", "group discounts", "website discount", "website discounts", "discounts work"]);
  const customCommercialRequest = hasAny(["custom discount", "special discount", "extra discount", "custom price", "special price", "price match", "bespoke price"]);
  ...
  if (hasAny(["price", "pricing", "cost", "quote", "how much", "estimate"])) return { classification: "pricing enquiry", assigned_agent: "theo", status: "auto_reply", action_taken: "Theo can explain pricing and calculate estimates only from configured pricing rules", approval_required: false, decision_reason: "Standard pricing enquiries can be answered safely when estimates are labelled and based on configured rules." };
  if (hasAny(["book", "booking", "attendees", "delegates", "drivers", "refresher booking", "refresher training", "full training", "multiple staff"])) return { classification: "booking information request", assigned_agent: "theo", status: "auto_reply", action_taken: "Theo can qualify the lead and gather course type, attendee numbers, location, refresher/full training need, expiry deadline and preferred timeframe", approval_required: false, decision_reason: "Safe booking information gathering does not confirm dates, prices or payment links." };
  return null;
}
...
async function saveInboundMessage(supabase, payload) {
  ...
  const metadata = { inbound_message_id: data.id, classification: data.classification, source: data.source, from_email: data.from_email, response_type: responseType, estimate_calculated: Boolean(estimate), training_page_referred: route.assigned_agent === "theo" && !route.approval_required, approval_required: route.approval_required, email_sent: false, lead_qualification: qualification, decision_reason: route.decision_reason || null, ...(theoTrace ? { theo_trace: theoTrace } : {}) };
