...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  const tabs = ["Dashboard", "Blogs", "Reviews", "Onboarding", "Members", "Training Compliance", "Reports & Exports", "Export Centre", "Depot Tokens", "Activity", "Media Manager", "Settings"];
  ...
  async function uploadTrainingEvidence(record, file) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        showMessage("error", result.error || "Could not upload evidence.");
        return;
      }
      await loadTrainingComplianceData({ quiet: true });
      setSelectedEvidenceRecordId(record.id);
      showMessage("success", "Evidence uploaded successfully.");
    } catch (error) {
      console.error("Evidence upload error:", error);
      showMessage("error", "Could not upload evidence.");
    } finally {
      setEvidenceBusyId("");
    }
  }

  function viewTrainingEvidence(record) {
    const evidence = evidenceForRecord(record.id);
    if (!evidence.length) {
      showMessage("error", "No evidence is attached to this record.");
      return;
    }
    setSelectedEvidenceRecordId(record.id);
    showMessage("success", "Evidence details opened.");
  }
  ...
  async function downloadTrainingEvidence(evidence) {
    ...
    try {
      ...
      if (!response.ok || !result.url) {
        ...
        showMessage("error", result.error || "Could not prepare evidence download.");
        return;
      }
      window.open(result.url, "_blank", "noopener,noreferrer");
      showMessage("success", "Evidence download opened.");
    } catch (error) {
      console.error("Evidence download error:", error);
      showMessage("error", "Could not prepare evidence download.");
    } finally {
      setEvidenceBusyId("");
    }
  }
  ...
  async function deleteTrainingEvidence(evidence) {
    if (!window.confirm(`Delete evidence file "${evidence.file_name}"?`)) return;
    setEvidenceBusyId(evidence.id);
    try {
      ...
      if (!response.ok) {
        ...
        showMessage("error", result.error || "Could not delete evidence.");
        return;
      }
      await loadTrainingComplianceData({ quiet: true });
      showMessage("success", "Evidence deleted successfully.");
    } catch (error) {
      console.error("Evidence delete error:", error);
      showMessage("error", "Could not delete evidence.");
    } finally {
      setEvidenceBusyId("");
    }
  }
  ...
  function generateCouncilCompliancePdf(organisationId) {
    ...
    autoTable(doc, {
      ...
      theme: "striped",
      head: [["Organisation", "Staff member", "Email", "Role", "Course", "Completed", "Expiry", "Status", "Evidence attached"]],
                  ...
                  <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                    ...
                    <div className="mt-5 w-full max-w-full overflow-x-auto rounded-xl border border-slate-200 bg-white">
                      <table className="w-full min-w-[980px] table-fixed text-left text-sm">
                        <thead className="bg-slate-100 text-slate-600"><tr><th className="w-[12%] p-3">Organisation</th><th className="w-[10%] p-3">Member</th><th className="w-[14%] p-3">Email</th><th className="w-[9%] p-3">Role</th><th className="w-[12%] p-3">Course</th><th className="w-[8%] p-3">Completed</th><th className="w-[8%] p-3">Expiry</th><th className="w-[8%] p-3">Status</th><th className="w-[8%] p-3">Evidence</th><th className="w-[11%] p-3">Actions</th></tr></thead>
                        <tbody>
                          {filteredTrainingRecords.map((record) => {
                            ...
                            return <tr key={record.id} className="border-t border-slate-200 align-top"><td className="break-words p-3">{record.organisation?.name || "Unknown"}</td><td className="break-words p-3 font-bold">{record.staffMember?.full_name || "Unknown"}</td><td className="break-all p-3">{record.staffMember?.email || ""}</td><td className="break-words p-3">{record.staffMember?.role || ""}</td><td className="break-words p-3">{record.course?.name || "Unknown"}</td><td className="p-3">{formatDisplayDate(record.date_completed)}</td><td className="p-3">{formatDisplayDate(record.expiry_date)}</td><td className="p-3"><span className={`inline-flex rounded-full px-2 py-1 text-xs font-black ${statusClass}`}>{record.status}</span></td><td className="p-3 font-bold">{recordEvidence.length ? "Yes" : "No"}</td><td className="p-3"><div className="grid min-w-[140px] gap-2"><input id={uploadInputId} type="file" accept="application/pdf,image/jpeg,image/png,image/webp" className="hidden" onChange={(e) => { uploadTrainingEvidence(record, e.target.files?.[0]); e.target.value = ""; }} /><button type="button" onClick={() => document.getElementById(uploadInputId)?.click()} disabled={evidenceBusyId === record.id} className="w-full rounded-lg bg-emerald-600 px-2 py-2 text-xs font-black text-white disabled:opacity-60">Upload Evidence</button><button type="button" onClick={() => viewTrainingEvidence(record)} className="w-full rounded-lg border border-slate-300 px-2 py-2 text-xs font-black text-slate-700">View Evidence</button><button type="button" onClick={() => downloadLatestTrainingEvidence(record)} disabled={!recordEvidence.length || evidenceBusyId === recordEvidence[0]?.id} className="w-full rounded-lg bg-slate-950 px-2 py-2 text-xs font-black text-white disabled:opacity-40">Download Evidence</button><button type="button" onClick={() => deleteLatestTrainingEvidence(record)} disabled={!recordEvidence.length || evidenceBusyId === recordEvidence[0]?.id} className="w-full rounded-lg bg-red-600 px-2 py-2 text-xs font-black text-white disabled:opacity-40">Delete Evidence</button><button type="button" onClick={() => deleteTrainingRecord(record)} className="w-full rounded-lg bg-red-700 px-2 py-2 text-xs font-black text-white">Delete Record</button></div></td></tr>;
                          })}
                        </tbody>
                      </table>
                    </div>
                    {selectedEvidenceRecordId ? (
                      <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-5">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          <div>
                            <h4 className="text-lg font-black">Training Evidence</h4>
                          ...
                          <button type="button" onClick={() => setSelectedEvidenceRecordId("")} className="rounded-xl border border-slate-300 px-4 py-2 text-sm font-black">Close Evidence</button>
                        </div>
                        <div className="mt-4 grid gap-3">
                          {evidenceForRecord(selectedEvidenceRecordId).length ? evidenceForRecord(selectedEvidenceRecordId).map((evidence) => (
                            <div key={evidence.id} className="grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 md:grid-cols-[minmax(0,1fr)_auto_auto] md:items-center">
                              ...
                              <button type="button" onClick={() => downloadTrainingEvidence(evidence)} disabled={evidenceBusyId === evidence.id} className="rounded-lg bg-slate-950 px-4 py-2 text-xs font-black text-white disabled:opacity-60">Download Evidence</button>
                              <button type="button" onClick={() => deleteTrainingEvidence(evidence)} disabled={evidenceBusyId === evidence.id} className="rounded-lg bg-red-600 px-4 py-2 text-xs font-black text-white disabled:opacity-60">Delete Evidence</button>
                            </div>
                          )) : <p className="rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-600">No evidence is attached to this record.</p>}
                        </div>
                      </div>
                    ) : null}
                  </section>
