...
const DEFAULT_SITE_SETTINGS = {
  ...
  stripeLink: import.meta.env?.VITE_STRIPE_PAYMENT_LINK || "https://buy.stripe.com/test_9B69ATd133zdfIhbcMdIA00",
...
function SubscribePage({ siteSettings }) {
  ...
  function proceedToPayment() {
    ...
    if (!siteSettings.stripeLink) {
      setError("Stripe payment link is not configured. Please add the live Stripe payment link in Back Office settings.");
      return;
    }
    window.location.href = siteSettings.stripeLink;
  }
  function Section({ title, items }) {
    return <section className="rounded-3xl border bg-white p-7 shadow-sm"><h2 className="text-2xl font-black">{title}</h2><div className="mt-5 grid gap-3">{items.map((item) => <p key={item} className="rounded-2xl bg-slate-50 p-4 text-sm font-semibold text-slate-700">✔ {item}</p>)}</div></section>;
  }
  return <main className="min-h-screen bg-slate-50 text-slate-950"><section className="bg-slate-950 px-6 py-20 text-white"><div className="mx-auto max-w-5xl"><p className="font-semibold text-emerald-300">Subscribe for Access</p><h1 className="mt-4 text-4xl font-black tracking-tight md:text-6xl">Compliance & Tracking System – £495 Setup Access</h1><p className="mt-6 max-w-4xl text-lg leading-8 text-slate-300">Give your transport operation the tools, structure and confidence to meet compliance standards and operate with complete peace of mind.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center"><button type="button" onClick={proceedToPayment} className="rounded-2xl bg-emerald-400 px-8 py-4 text-lg font-black text-slate-950 shadow-xl">Secure Access & Setup – £495</button><p className="text-sm font-semibold text-slate-300">One-time setup access. No hidden fees.</p></div>{error ? <p className="mt-5 rounded-xl bg-red-100 p-4 text-sm font-semibold text-red-800">{error}</p> : null}</div></section><div className="mx-auto grid max-w-7xl gap-6 px-6 py-16 lg:grid-cols-2"><Section title="What you get" items={valuePoints} /><Section title="Why this matters" items={reasons} /><Section title="Bespoke service" items={bespoke} /><Section title="Support and aftercare" items={support} /></div><section className="px-6 pb-20"><div className="mx-auto max-w-5xl rounded-3xl bg-emerald-500 p-8 text-center text-slate-950"><p className="text-2xl font-black md:text-4xl">Everything you need to run a compliant, accountable and professional transport operation — all in one place.</p><button type="button" onClick={proceedToPayment} className="mt-8 rounded-2xl bg-slate-950 px-8 py-4 text-lg font-black text-white">Proceed to Payment (£495)</button><p className="mt-3 text-sm font-semibold">One-time setup access. No hidden fees.</p></div></section></main>;
}
...
function LoginPage({ setPage, setLoggedInMember }) {
  ...
  return <main className="min-h-screen bg-slate-50 px-6 py-20"><div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.9fr_1.1fr]"><div><p className="font-semibold text-emerald-700">Secure Member Login</p><h1 className="mt-3 text-4xl font-black tracking-tight md:text-6xl">Access your organisation’s compliance portal.</h1><p className="mt-5 leading-8 text-slate-600">Members sign in with a secure email verification code. No shared demo passwords and no plain text passwords are used.</p><div className="mt-8 rounded-3xl border border-emerald-200 bg-emerald-50 p-6 text-sm leading-7 text-emerald-900"><strong>Secure access:</strong> enter your member email, then use the verification code sent to your inbox.</div></div><div className="rounded-[2rem] border border-slate-200 bg-white p-8 shadow-xl">{step === "credentials" ? <form onSubmit={handleCredentials} className="grid gap-4"><h2 className="text-3xl font-black">Member Login</h2>{error ? <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p> : null}<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="rounded-xl border p-4" placeholder="Email address" required /><button type="submit" disabled={isLoading} className="rounded-xl bg-slate-950 p-4 font-black text-white disabled:opacity-60">{isLoading ? "Sending..." : "Send Secure Login Code"}</button></form> : <form onSubmit={handleVerification} className="grid gap-4"><h2 className="text-3xl font-black">Enter verification code</h2><p className="text-sm text-slate-500">Check your email for the secure login code.</p>{error ? <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p> : null}<input value={code} onChange={(e) => setCode(e.target.value)} className="rounded-xl border p-4 text-center text-2xl tracking-[0.3em]" placeholder="123456" required /><button type="submit" disabled={isLoading} className="rounded-xl bg-emerald-600 p-4 font-black text-white disabled:opacity-60">{isLoading ? "Verifying..." : "Verify & Enter Portal"}</button><button type="button" onClick={() => setStep("credentials")} className="rounded-xl border p-4 font-bold text-slate-700">Back</button></form>}</div></div></main>;
}
function MemberDashboardPage({ member, setPage, session }) {
  useEffect(() => {
    if (!session) setPage("Login");
  }, [session, setPage]);
  if (!session) return <main className="min-h-screen bg-slate-50 px-6 py-20 text-center"><h1 className="text-4xl font-bold">Login required</h1><p className="mt-3 text-slate-600">Please use secure member login to access this dashboard.</p><button type="button" onClick={() => setPage("Login")} className="mt-8 rounded-xl bg-slate-950 px-6 py-3 font-bold text-white">Go to Login</button></main>;
          ...
          <section className="rounded-3xl bg-white p-6 text-slate-950 shadow-xl">
            ...
            {activeTab === "Blogs" ? (
              <div>
                ...
                <div className="mt-8 grid gap-3">
                  {posts.map((post) => <div key={post.id || `${post.title}-${post.status}`} className="rounded-2xl bg-slate-50 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-bold text-emerald-700">{post.tag} - {post.status}</p><h3 className="mt-1 font-black">{post.title}</h3><p className="mt-2 text-sm text-slate-600">{post.text}</p></div><div className="flex shrink-0 flex-wrap gap-2"><button type="button" onClick={() => updateBlogStatus(post, "Published")} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white">Publish</button><button type="button" onClick={() => updateBlogStatus(post, "Draft")} className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-bold text-slate-700">Draft</button><button type="button" onClick={() => deleteBlog(post)} className="rounded-lg bg-red-600 px-3 py-2 text-xs font-bold text-white">Delete</button></div></div></div>)}
                </div>
              </div>
            ) : null}
            ...
            {activeTab === "Reviews" ? (
              <div>
                ...
                <div className="mt-8 grid gap-3">
                  {reviews.map((review) => <div key={review.id || `${review.name}-${review.org}-${review.status}`} className="rounded-2xl bg-slate-50 p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="text-sm font-bold text-amber-600">{review.rating} - {review.status}</p><h3 className="mt-1 font-black">{review.name}</h3><p className="text-sm text-slate-500">{review.org}</p><p className="mt-2 text-sm text-slate-600">{review.text}</p></div><div className="flex shrink-0 flex-wrap gap-2"><button type="button" onClick={() => updateReviewStatus(review, "Published")} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-bold text-white">Publish</button><button type="button" onClick={() => updateReviewStatus(review, "Draft")} className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-bold text-slate-700">Draft</button><button type="button" onClick={() => deleteReview(review)} className="rounded-lg bg-red-600 px-3 py-2 text-xs font-bold text-white">Delete</button></div></div></div>)}
                </div>
              </div>
            ) : null}
            ...
            {activeTab === "Onboarding" ? (
              <div>
                ...
                <div className="mt-6 grid gap-3">
                  {onboarding.map((item) => <div key={item.id || item.email} className="rounded-2xl bg-slate-50 p-4"><div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between"><div><h3 className="font-black">{item.organisation || "Organisation pending"}</h3><p className="text-sm text-slate-600">{item.contact_name || item.contact || "Contact pending"} - {item.email}</p><p className="mt-2 text-sm">Tools requested: {item.tools_required || item.modules || "Not specified"}</p><p className="text-sm">Preferred login: {item.preferred_login_method || "Email verification code"}</p><p className="text-sm">Depots/sites: {item.depots || "Not specified"} | Road staff: {item.road_staff || "Not specified"}</p>{item.notes ? <p className="mt-2 text-sm text-slate-600">{item.notes}</p> : null}{item.stripe_session_id ? <p className="mt-2 break-all text-xs text-slate-500">Stripe session: {item.stripe_session_id}</p> : null}</div><p className="rounded-xl bg-emerald-50 px-3 py-2 text-sm font-bold text-emerald-700">{item.status || "New"}</p></div></div>)}
                </div>
              </div>
            ) : null}
...
export default function App() {
  ...
  function pageFromLocation() {
    if (window.location.pathname.toLowerCase().endsWith("/subscribe")) return "Subscribe";
    if (window.location.pathname.toLowerCase().endsWith("/payment-success")) return "PaymentSuccess";
    if (window.location.pathname.toLowerCase().endsWith("/member-welcome")) return "MemberWelcome";
  ...
  function setPage(nextPage) {
    ...
    const path = nextPage === "Subscribe" ? "/subscribe" : nextPage === "PaymentSuccess" ? "/payment-success" : nextPage === "MemberWelcome" ? "/member-welcome" : "/";
    const nextUrl = `${path}${window.location.search}${hash && nextPage !== "Home" && nextPage !== "Subscribe" ? `#${hash}` : ""}`;
    window.history.replaceState(null, "", nextUrl);
  }

  useEffect(() => {
    function handleHashChange() { setPageState(pageFromLocation()); }
