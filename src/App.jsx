...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  const aiAgentCards = [
    { key: "ava", name: "Ava", role: "Compliance Agent", status: "Active monitoring", actionsToday: agentLogsTodayFor("ava").length, pendingTasks: expiringWithinThirtyDays.length, approvalsRequired: 0, lastActivity: agentLogsFor("ava")[0]?.created_at ? formatDisplayDateTime(agentLogsFor("ava")[0].created_at) : "No activity yet" },
    { key: "mia", name: "Mia", role: "Outreach Agent", status: "Approved reminders", actionsToday: agentLogsTodayFor("mia").length, pendingTasks: expiringWithinThirtyDays.length, approvalsRequired: 0, lastActivity: agentLogsFor("mia")[0]?.created_at ? formatDisplayDateTime(agentLogsFor("mia")[0].created_at) : "No reminders sent yet" },
    { key: "theo", name: "Theo", role: "Booking Agent", status: "Approval required", actionsToday: agentLogsTodayFor("theo").length, pendingTasks: onboarding.filter((item) => (item.status || "New") !== "Completed").length, approvalsRequired: onboarding.length, lastActivity: agentLogsFor("theo")[0]?.created_at ? formatDisplayDateTime(agentLogsFor("theo")[0].created_at) : "Awaiting human date approval" },
    { key: "nia", name: "Nia", role: "Content Agent", status: "Structured placeholder", actionsToday: agentLogsTodayFor("nia").length, pendingTasks: posts.filter((post) => post.status === "Draft").length, approvalsRequired: 0, lastActivity: agentLogsFor("nia")[0]?.created_at ? formatDisplayDateTime(agentLogsFor("nia")[0].created_at) : "Ready for content drafts" },
    { key: "ellis", name: "Ellis", role: "Email Agent", status: "Structured placeholder", actionsToday: agentLogsTodayFor("ellis").length, pendingTasks: trainingReminderLogs.length, approvalsRequired: 0, lastActivity: agentLogsFor("ellis")[0]?.created_at ? formatDisplayDateTime(agentLogsFor("ellis")[0].created_at) : "Ready for inbox filtering" },
    { key: "rory", name: "Rory", role: "Research Agent", status: "Prospect research", actionsToday: agentLogsTodayFor("rory").length, pendingTasks: pendingProspects.length, approvalsRequired: pendingProspects.length, lastActivity: agentLogsFor("rory")[0]?.created_at ? formatDisplayDateTime(agentLogsFor("rory")[0].created_at) : "Ready to organise prospects" }
  ];
  ...
  async function runAvaMiaWorkflow() {
    ...
    try {
      ...
      setActivity((current) => [`Ava/Mia workflow complete: ${result.remindersSent} reminder(s) sent, ${result.skipped} skipped`, ...current]);
      showMessage("success", `Ava/Mia workflow complete. ${result.remindersSent} reminder(s) sent.`);
    } catch (error) {
            ...
            {activeTab === "AI Operations" ? (
              <div>
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <button type="button" onClick={runAvaMiaWorkflow} disabled={agentWorkflowRunning} className="rounded-xl bg-emerald-600 px-5 py-3 text-sm font-black text-white disabled:cursor-wait disabled:opacity-60">{agentWorkflowRunning ? "Running Ava/Mia..." : "Run Ava/Mia Refresher Workflow"}</button>
                </div>
                ...
                <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                  {aiAgentCards.map((agent) => (
                    <article key={agent.key} className="rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                      ...
                      <p className="mt-1 text-sm font-bold text-slate-600">{agent.role}</p>
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                  ...
                  <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                    {aiAgentCards.map((agent) => <div key={`${agent.key}-summary`} className="rounded-xl bg-white p-4 text-sm font-semibold text-slate-700"><p className="font-black text-slate-950">{agent.name}</p><p className="mt-2">Status: {agent.status}. Pending: {agent.pendingTasks}. Approvals: {agent.approvalsRequired}.</p></div>)}
                  </div>
                </section>
