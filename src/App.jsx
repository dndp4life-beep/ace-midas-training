...
const DEFAULT_MEDIA_SETTINGS = {
  ...
  midasCard: { label: "MiDAS card image", imageUrl: images.driverSeat, altText: "MiDAS minibus driver training", x: 50, y: 50, zoom: 1 },
  patsCard: { label: "PATS card image", imageUrl: images.interior, altText: "Passenger assistant training", x: 50, y: 50, zoom: 1 },
  firstAidCard: { label: "First Aid card image", imageUrl: images.firstAid, altText: "First aid training", x: 50, y: 50, zoom: 1 },
...
function getCourseMediaSlot(courseTitle) {
  if (courseTitle?.includes("PATS")) return "patsCard";
  if (courseTitle?.includes("First Aid") || courseTitle?.includes("Children")) return "firstAidCard";
  return "midasCard";
}
...
function HomePage({ setPage, mediaSettings }) {
  const cards = [
    { title: "MiDAS Training", text: "Driver awareness training for minibus and passenger transport operations.", slot: "midasCard" },
    { title: "PATS Training", text: "Passenger assistant training for staff supporting children and vulnerable passengers.", slot: "patsCard" },
...
function LoginPage({ setPage, setLoggedInMember }) {
  ...
  async function handleCredentials(e) {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(SEND_LOGIN_CODE_API_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleVerification(e) {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(VERIFY_LOGIN_CODE_API_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return <main className="min-h-screen bg-slate-50 px-6 py-20"><div className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[0.9fr_1.1fr]"><div><p className="font-semibold text-emerald-700">Secure Member Login</p><h1 className="mt-3 text-4xl font-black tracking-tight md:text-6xl">Access your organisation’s compliance portal.</h1><p className="mt-5 leading-8 text-slate-600">Members sign in with a secure 6-digit code sent to their approved member email. Access is only available for active subscribed organisations.</p><div className="mt-8 rounded-3xl border border-emerald-200 bg-emerald-50 p-6 text-sm leading-7 text-emerald-900"><strong>Secure access:</strong> enter your approved email, then use the verification code sent to your inbox. No shared live passwords are used.</div></div><div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">{step === "credentials" ? <form onSubmit={handleCredentials} className="grid gap-4"><h2 className="text-3xl font-black">Member Login</h2>{error ? <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p> : null}<input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="rounded-xl border p-4" placeholder="Approved member email" required /><button type="submit" disabled={isLoading} className="rounded-xl bg-slate-950 p-4 font-black text-white disabled:opacity-60">{isLoading ? "Sending..." : "Send Secure Login Code"}</button></form> : <form onSubmit={handleVerification} className="grid gap-4"><h2 className="text-3xl font-black">Enter verification code</h2><p className="text-sm text-slate-500">Check your email for the secure 6-digit login code. It expires after 10 minutes.</p>{notice ? <p className="rounded-xl bg-emerald-50 p-3 text-sm font-semibold text-emerald-700">{notice}</p> : null}{error ? <p className="rounded-xl bg-red-50 p-3 text-sm text-red-700">{error}</p> : null}<input value={code} onChange={(e) => setCode(e.target.value.replace(/\D/g, "").slice(0, 6))} className="rounded-xl border p-4 text-center text-2xl tracking-[0.3em]" placeholder="Enter code" required /><button type="submit" disabled={isLoading} className="rounded-xl bg-emerald-600 p-4 font-black text-white disabled:opacity-60">{isLoading ? "Verifying..." : "Verify & Enter Portal"}</button><button type="button" onClick={() => setStep("credentials")} className="rounded-xl border p-4 font-bold text-slate-700">Back</button></form>}</div></div></main>;
}
...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  const tabs = ["Dashboard", "Blogs", "Reviews", "Onboarding", "Members", "Depot Tokens", "Activity", "Media Manager", "Settings"];
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[240px_1fr]">
          ...
          <section className="rounded-2xl bg-white p-6 text-slate-950 shadow-sm">
            ...
            {activeTab === "Members" ? (
              <div>
                ...
                <form onSubmit={createMemberAccount} className="mt-6 grid gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                  ...
                  <button type="submit" className="rounded-xl bg-slate-950 p-4 font-black text-white">Create Member</button>
                </form>
            ...
            {activeTab === "Media Manager" ? (
              <div>
                <h2 className="text-3xl font-black">Media Manager</h2>
                ...
                <div className="mt-6 grid gap-5">
                  {Object.entries(DEFAULT_MEDIA_SETTINGS).map(([slot, defaults]) => {
                    const media = getMediaSlot(mediaSettings, slot);
                    return (
