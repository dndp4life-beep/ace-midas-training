...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  async function scoreRoryProspect(prospect) {
    ...
    try {
      ...
      setFollowUpTasks(result.followUps || followUpTasks);
      setAgentActivityLogs(result.agentLogs || agentActivityLogs);
      showMessage("success", "Prospect scored and outreach brief updated.");
    } catch (error) {
  ...
  async function runRoryResearchNow() {
    ...
    try {
      const { response, result } = await callAdminAction("run-rory-prospect-research", { searchTheme: rorySearchTheme, runType: "manual" });
      if (!response.ok) {
              ...
              <div>
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    {roryResearchStatus?.error ? <p className="mt-3 rounded-xl bg-red-50 p-3 text-sm font-bold text-red-700">{safeText(roryResearchStatus.error, "Rory research failed.")}</p> : null}
                    <div className="mt-5">
                  ...
                  <div className="mt-5 grid gap-3">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                      ...
                      <p className="rounded-xl bg-blue-50 px-4 py-3 text-sm font-black text-blue-800">{highPriorityProspects.length} high priority prospect(s)</p>
                    </div>
