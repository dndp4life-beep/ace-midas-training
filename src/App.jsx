...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  const [prospectForm, setProspectForm] = useState({ organisation_name: "", website: "", location: "", sector: "schools", likely_training_need: "", contact_email: "", phone: "", decision_maker_name: "", source_url: "", notes: "", priority: "medium", relevance_reason: "", review_status: "pending_review" });
  ...
  const [aiRules, setAiRules] = useState({
    ava: { approvalRequired: false, allowedActions: "Internal-only compliance monitoring, expiry risk detection, missing certificate review, failed reminder review, and admin daily/weekly summary emails", forbiddenActions: "Email customers, confirm bookings, alter training records, delete evidence, send payment links", escalationTriggers: "Expired training, missing certificates, failed reminders, urgent organisations, records expiring within 7 days" },
    mia: { approvalRequired: false, allowedActions: "Send standard refresher reminders and follow-up emails within approved wording", forbiddenActions: "Confirm dates, promise availability, change pricing, confirm bookings, send custom payment links", escalationTriggers: "Any booking/date/payment wording or non-standard customer request" },
    theo: { approvalRequired: false, allowedActions: "Answer safe booking process questions, explain group or onsite training in general terms, gather attendee numbers, course type, location and preferred timeframe, refer customers to the training page, and calculate indicative estimates only from approved website pricing rules", forbiddenActions: "Confirm training dates, promise availability, change bookings, cancel bookings, agree custom prices, offer non-standard discounts, send/request custom Stripe links", escalationTriggers: "Exact dates, confirmed availability, booking changes, cancellations, pricing exceptions, custom discounts, custom prices, payment links" },
    nia: { approvalRequired: false, allowedActions: "Create promotional content ideas, draft posts, summarise content opportunities", forbiddenActions: "Publish public pages without review, change pricing, alter legal copy", escalationTriggers: "Legal wording, safeguarding wording, council-facing claims" },
    ellis: { approvalRequired: false, allowedActions: "Categorise emails, summarise useful leads, ignore low-value B2B spam", forbiddenActions: "Send replies, confirm bookings, attach private files", escalationTriggers: "Complaint, safeguarding concern, payment dispute, booking confirmation" },
    rory: { approvalRequired: false, allowedActions: "Research publicly available UK organisations, organise structured prospect records, explain relevance and priority", forbiddenActions: "Scrape private data, buy lead lists, send outreach, collect personal data that is not publicly available", escalationTriggers: "Unclear data source, personal contact details, outreach request, high-priority council or safeguarding prospect" }
  });
  ...
  const tabs = ["Dashboard", "Blogs", "Reviews", "Onboarding", "Members", "Training Compliance", "Reports & Exports", "Export Centre", "Depot Tokens", "Activity", "AI Operations", "Workflow Debug Trace", "Media Manager", "Settings"];
  ...
  const agentLogsToday = agentActivityLogs.filter((log) => String(log.created_at || "").startsWith(todayKey));
  const agentLogsFor = (agentKey) => agentActivityLogs.filter((log) => log.agent_key === agentKey);
  const agentLogsTodayFor = (agentKey) => agentLogsToday.filter((log) => log.agent_key === agentKey);
  const pendingProspects = prospects.filter((prospect) => (prospect.review_status || "pending_review") === "pending_review");
  const highPriorityProspects = prospects.filter((prospect) => prospect.priority === "high" && (prospect.review_status || "pending_review") === "pending_review");
  const theoReplyApprovals = replyIntake.filter((reply) => reply.approval_required && (reply.approval_status || "pending") === "pending");
  const aiAgentCards = [
    ...
    { key: "theo", role: "Booking Agent", status: "Approval required", actionsToday: agentLogsTodayFor("theo").length, pendingTasks: onboarding.filter((item) => (item.status || "New") !== "Completed").length + theoReplyApprovals.length, approvalsRequired: onboarding.length + theoReplyApprovals.length, lastActivity: agentLogsFor("theo")[0]?.created_at ? formatDisplayDateTime(agentLogsFor("theo")[0].created_at) : "Awaiting human date approval" },
    { key: "nia", role: "Content Agent", status: "Structured placeholder", actionsToday: agentLogsTodayFor("nia").length, pendingTasks: posts.filter((post) => post.status === "Draft").length, approvalsRequired: 0, lastActivity: agentLogsFor("nia")[0]?.created_at ? formatDisplayDateTime(agentLogsFor("nia")[0].created_at) : "Ready for content drafts" },
  ...
  const aiApprovalQueue = [
    { id: "theo-booking-date-approval", agent: "Theo", task: "Review preferred training dates before sending final confirmation", risk: "Theo cannot confirm dates automatically", count: onboarding.length },
    { id: "theo-payment-link-approval", agent: "Theo", task: "Review any custom payment link or custom price request", risk: "Custom pricing/payment decisions require human approval", count: onboarding.filter((item) => String(item.notes || "").toLowerCase().includes("price") || String(item.notes || "").toLowerCase().includes("payment")).length },
  ...
  const avaDailySummariesToday = agentLogsTodayFor("ava").filter((log) => log.action_type === "ava_daily_summary_sent").length;
  const avaWeeklySummariesToday = agentLogsTodayFor("ava").filter((log) => log.action_type === "ava_weekly_summary_sent").length;
  ...
  const contentDraftsToday = agentLogsTodayFor("nia").filter((log) => log.action_type === "content_draft_created").length;
  ...
  const inboundToday = inboundMessages.filter((item) => String(item.created_at || "").startsWith(todayKey));
  ...
  const theoDecisionTraceLogs = agentActivityLogs.filter((log) => log.agent_key === "theo" && (log.action_type === "theo_decision_trace" || log.metadata?.theo_trace));
  const theoTraceForInbound = (inboundId) => theoDecisionTraceLogs.find((log) => (log.metadata?.theo_trace?.inbound_message_id || log.metadata?.inbound_message_id) === inboundId);
  const selectedTheoTraceLog = selectedTheoTraceId ? theoTraceForInbound(selectedTheoTraceId) : theoDecisionTraceLogs[0];
  const selectedTheoTrace = selectedTheoTraceLog?.metadata?.theo_trace || null;
  const selectedTheoInbound = selectedTheoTrace ? inboundMessages.find((message) => message.id === selectedTheoTrace.inbound_message_id) : null;
  ...
  async function saveRoryProspect() {
    if (!prospectForm.organisation_name.trim() || !prospectForm.source_url.trim()) {
      showMessage("error", "Organisation name and source URL are required.");
      return;
    }
    setIsSaving(true);
    try {
      ...
      setAgentActivityLogs(result.agentLogs || agentActivityLogs);
      setProspectForm({ organisation_name: "", website: "", location: "", sector: "schools", likely_training_need: "", contact_email: "", phone: "", decision_maker_name: "", source_url: "", notes: "", priority: "medium", relevance_reason: "", review_status: "pending_review" });
  ...
  useEffect(() => {
    if (!unlocked || !["Training Compliance", "Export Centre", "Reports & Exports"].includes(activeTab)) return;
              ...
              <div>
                    ...
                    <article key={agent.key} className="rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="mt-1 text-sm font-bold text-slate-600">{agent.title}</p>
                        </div>
                      </div>
                      <p className="mt-2 rounded-full bg-white px-3 py-1 text-xs font-black text-emerald-700">{agent.status}</p>
                      <div className="mt-4 grid gap-2 text-sm font-semibold text-slate-700">
                        <p>Actions today: <span className="font-black text-slate-950">{agent.actionsToday}</span></p>
                        <p>Pending tasks: <span className="font-black text-slate-950">{agent.pendingTasks}</span></p>
                        <p>Approval required: <span className="font-black text-slate-950">{agent.approvalsRequired}</span></p>
                        <p>Last activity: <span className="font-black text-slate-950">{agent.lastActivity}</span></p>
                      </div>
                    </article>
                  ))}
                </div>
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                  ...
                  <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                    {aiAgentCards.map((agent) => <div key={`${agent.key}-summary`} className="rounded-xl bg-white p-4 text-sm font-semibold text-slate-700"><p className="font-black text-slate-950">{agent.name}</p><p className="mt-1 text-xs font-bold text-slate-500">{agent.title}</p><p className="mt-2">Status: {agent.status}. Pending: {agent.pendingTasks}. Approvals: {agent.approvalsRequired}.</p></div>)}
                  </div>
                </section>
                ...
                <section className="mt-6 rounded-2xl border border-emerald-200 bg-emerald-50 p-5 shadow-sm">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      ...
                      <button type="button" onClick={() => sendAvaSummary("daily")} disabled={Boolean(avaSummaryBusy)} className="rounded-xl bg-emerald-700 px-5 py-3 text-sm font-black text-white disabled:opacity-60">{avaSummaryBusy === "daily" ? "Sending..." : "Send End-of-Day Summary"}</button>
                      <button type="button" onClick={() => sendAvaSummary("weekly")} disabled={Boolean(avaSummaryBusy)} className="rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white disabled:opacity-60">{avaSummaryBusy === "weekly" ? "Sending..." : "Send End-of-Week Summary"}</button>
                    </div>
                  </div>
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    ...
                    <div className="rounded-xl bg-blue-50 px-4 py-3 text-sm font-black text-blue-800">{inboundMessages.length} captured</div>
                  </div>
                  ...
                  <div className="mt-5 overflow-x-auto">
                    <table className="min-w-[980px] w-full text-left text-sm">
                      ...
                      <tbody>{inboundMessages.length ? inboundMessages.map((item) => <tr key={item.id} className="border-t border-slate-200"><td className="p-3">{formatDisplayDateTime(item.created_at)}</td><td className="p-3">{item.from_name || item.from_email || "Unknown"}<p className="text-xs text-slate-500">{item.organisation}</p></td><td className="p-3">{item.subject || "No subject"}</td><td className="p-3 font-bold">{item.classification}</td><td className="p-3 font-bold uppercase">{item.assigned_agent}</td><td className="p-3">{item.status}</td><td className="p-3">{item.action_taken}</td><td className="p-3">{item.assigned_agent === "theo" ? <button type="button" onClick={() => openTheoDecisionTrace(item.id)} className="rounded-lg bg-slate-950 px-3 py-2 text-xs font-black text-white">View Theo Decision Trace</button> : <span className="text-xs font-bold text-slate-400">Not Theo</span>}</td></tr>) : <tr><td className="p-4 text-sm font-bold text-slate-600" colSpan={8}>No inbound messages captured yet.</td></tr>}</tbody>
                    </table>
                  </div>
                </section>
                      ...
                      return <article key={reply.id} className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
                        <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-start">
                          <div>
                            ...
                            <p className="mt-2 text-sm leading-relaxed text-slate-700">{reply.message}</p>
                            <p className="mt-2 text-xs font-black text-amber-800">Classification: {reply.classification} | Requested action: {reply.requested_action}</p>
                          </div>
                        </div>
                        <div className="mt-4 grid gap-3 rounded-xl bg-white p-4 text-sm font-semibold text-slate-700 md:grid-cols-2 lg:grid-cols-4">
                          <p>Requested course: <span className="font-black text-slate-950">{reply.requested_course || relatedRecord?.course?.name || "Not known"}</span></p>
                          <p>Attendees: <span className="font-black text-slate-950">{reply.attendees || "Not known"}</span></p>
                          <p>Location: <span className="font-black text-slate-950">{reply.location || "Not known"}</span></p>
                          <p>Preferred dates: <span className="font-black text-slate-950">{reply.preferred_dates || "Not known"}</span></p>
                          <p>Urgency: <span className="font-black text-slate-950">{reply.urgency || "Medium"}</span></p>
                        ...
                        <div className="mt-4 grid gap-3 rounded-xl border border-amber-200 bg-white p-4 lg:grid-cols-2">
                          ...
                          {reply.draft_response ? <div className="rounded-xl bg-slate-50 p-3 text-sm font-semibold leading-relaxed text-slate-700 lg:col-span-2"><p className="font-black text-slate-950">Theo draft response</p><pre className="mt-2 whitespace-pre-wrap font-sans">{reply.draft_response}</pre></div> : null}
                        </div>
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <h3 className="text-xl font-black">Rory Prospect Review List</h3>
                      <p className="mt-2 text-sm font-semibold leading-relaxed text-slate-600">Rory stores publicly sourced UK prospects for Marvin to review before any outreach. No automatic emails are sent from this section.</p>
                    </div>
                    <div className="rounded-xl bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-800">{pendingProspects.length} pending review</div>
                  </div>

                  <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <h4 className="font-black">Add researched prospect</h4>
                    <div className="mt-4 grid gap-3 lg:grid-cols-3">
                    ...
                    <button type="button" onClick={saveRoryProspect} disabled={isSaving} className="mt-4 rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white disabled:opacity-60">{isSaving ? "Saving..." : "Add Prospect for Review"}</button>
                  </div>

                  <div className="mt-5 grid gap-3">
                    {prospects.length ? prospects.map((prospect) => <article key={prospect.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                          <h4 className="text-lg font-black">{prospect.organisation_name}</h4>
                          <p className="mt-1 text-sm font-semibold text-slate-700">{prospect.sector || "Sector not recorded"} | {prospect.location || "Location not recorded"} | {prospect.priority || "medium"} priority</p>
                          <p className="mt-2 text-sm leading-relaxed text-slate-600">{prospect.relevance_reason || prospect.likely_training_need || "Relevance reason not recorded."}</p>
                          <p className="mt-2 break-all text-xs font-bold text-slate-500">Source: {prospect.source_url}</p>
                        </div>
                        <div className="flex flex-wrap gap-2">
                      ...
                      <div className="mt-3 grid gap-2 text-xs font-bold text-slate-600 sm:grid-cols-2 lg:grid-cols-4">
                        <p>Email: {prospect.contact_email || "Not publicly found"}</p>
                        <p>Phone: {prospect.phone || "Not publicly found"}</p>
                        <p>Decision-maker: {prospect.decision_maker_name || "Not publicly found"}</p>
                        <p>Status: {String(prospect.review_status || "pending_review").replace("_", " ")}</p>
                      </div>
                    </article>) : <p className="rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-600">No prospects have been added yet.</p>}
                  </div>
                </section>
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="mt-5 grid gap-3">
                    {agentActivityLogs.length ? agentActivityLogs.slice(0, 20).map((log) => <div key={log.id} className="rounded-xl border border-slate-200 bg-slate-50 p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-black">{log.agent_name}</p><p className="mt-1 text-sm font-semibold text-slate-700">{log.summary}</p></div><p className="text-xs font-bold text-slate-500">{formatDisplayDateTime(log.created_at)}</p></div><p className="mt-2 text-xs font-bold text-emerald-700">Status: {log.status} | Approval required: {log.approval_required ? "Yes" : "No"}</p></div>) : <p className="rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-600">No agent activity has been logged yet.</p>}
                  </div>
                </section>
                      ...
                      <div key={`${agent.key}-identity`} className="rounded-2xl border border-slate-200 bg-white p-4">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs font-bold text-slate-500">{agent.title}</p>
                          </div>
                        </div>
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                  ...
                  <div className="mt-5 grid gap-4">
                    {aiAgentCards.map((agent) => {
                      const rules = aiRules[agent.key];
                      return <div key={`${agent.key}-rules`} className="rounded-2xl border border-slate-200 bg-white p-4"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><h4 className="text-lg font-black">{agent.name}</h4><label className="flex items-center gap-2 text-sm font-black"><input type="checkbox" checked={rules.approvalRequired} onChange={(e) => updateAiRule(agent.key, "approvalRequired", e.target.checked)} /> Approval required</label></div><div className="mt-4 grid gap-4 lg:grid-cols-3"><label className="grid gap-2 text-sm font-bold text-slate-700">Allowed actions<textarea value={rules.allowedActions} onChange={(e) => updateAiRule(agent.key, "allowedActions", e.target.value)} className="rounded-xl border border-slate-200 p-3 font-normal" rows={3} /></label><label className="grid gap-2 text-sm font-bold text-slate-700">Forbidden actions<textarea value={rules.forbiddenActions} onChange={(e) => updateAiRule(agent.key, "forbiddenActions", e.target.value)} className="rounded-xl border border-slate-200 p-3 font-normal" rows={3} /></label><label className="grid gap-2 text-sm font-bold text-slate-700">Escalation triggers<textarea value={rules.escalationTriggers} onChange={(e) => updateAiRule(agent.key, "escalationTriggers", e.target.value)} className="rounded-xl border border-slate-200 p-3 font-normal" rows={3} /></label></div></div>;
                    })}
                  </div>
                </section>
              </div>
            ...
            {activeTab === "Workflow Debug Trace" ? (
              <div>
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <button type="button" onClick={loadTrainingCompliance} className="rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white">Refresh Trace Data</button>
                </div>
                  ...
                  <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                    ...
                    <div className="mt-4 grid gap-3">
                      {theoDecisionTraceLogs.length ? theoDecisionTraceLogs.slice(0, 12).map((log) => {
                        const trace = log.metadata?.theo_trace || {};
                        const inboundId = trace.inbound_message_id || log.metadata?.inbound_message_id || "";
                        return <button key={log.id} type="button" onClick={() => setSelectedTheoTraceId(inboundId)} className={`rounded-xl border p-4 text-left transition ${selectedTheoTraceId === inboundId ? "border-emerald-500 bg-white shadow-sm" : "border-slate-200 bg-white hover:border-emerald-300"}`}>
                          <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                            <div>
                              <p className="font-black">{trace.classification_result || log.metadata?.classification || "Theo decision"}</p>
                              <p className="mt-1 text-xs font-bold text-slate-500">Inbound: {inboundId || "Not linked"}</p>
                            </div>
                    ...
                    {selectedTheoTrace ? (
                      <div className="mt-4 grid gap-4">
                        <div className="rounded-xl bg-slate-50 p-4">
                          ...
                          <p className="mt-2 text-sm font-semibold leading-relaxed text-slate-700">{selectedTheoInbound?.message_body || "Original inbound message is not available in the current loaded data."}</p>
                        </div>
                        <div className="grid gap-3 md:grid-cols-2">
                          <div className="rounded-xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-500">Inbound message ID</p><p className="mt-2 break-all text-sm font-black">{selectedTheoTrace.inbound_message_id || "Not recorded"}</p></div>
                          <div className="rounded-xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-500">Classification result</p><p className="mt-2 text-sm font-black">{selectedTheoTrace.classification_result || "Not recorded"}</p></div>
                          <div className="rounded-xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-500">Assigned agent</p><p className="mt-2 text-sm font-black uppercase">{selectedTheoTrace.assigned_agent || "Not recorded"}</p></div>
                          ...
                          <div className="rounded-xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-500">Resend response status</p><p className="mt-2 text-sm font-black">{selectedTheoTrace.resend_response_status || "Not attempted / not recorded"}</p></div>
                          ...
                          <div className="rounded-xl bg-slate-50 p-4"><p className="text-xs font-black uppercase text-slate-500">Thrown error</p><p className="mt-2 break-words text-sm font-semibold text-red-700">{selectedTheoTrace.thrown_error || "None"}</p></div>
                        </div>
