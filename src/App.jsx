...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings, loginNonce }) {
  ...
  const [ellisEmails, setEllisEmails] = useState([]);
  const [ellisTasks, setEllisTasks] = useState([]);
  ...
  const safeEllisEmails = asArray(ellisEmails).map((email) => ({ ...asObject(email), reasoning_metadata: asObject(asObject(email).reasoning_metadata) }));
  const safeEllisTasks = asArray(ellisTasks).map((task) => asObject(task));
  ...
  const filteredOpportunities = safeOpportunities.filter((opportunity) => {
    if (!opportunityFilters.status && opportunity.status === "dormant") return false;
  ...
  const filteredEllisEmails = safeEllisEmails.filter((email) => {
    const search = ellisSearch.trim().toLowerCase();
    if (ellisCategoryFilter && email.category !== ellisCategoryFilter) return false;
    if (ellisPriorityFilter && email.priority !== ellisPriorityFilter) return false;
  ...
  function previewQueuedMiaOutreach(item) {
    ...
    setMiaEmailPreview({
      ...
      subject: item.email_subject || "Mia outreach email",
      html: item.email_html || "<p>No prepared email preview is available.</p>",
  ...
  async function approveAndSendQueuedMiaOutreach(item) {
    ...
    setIsSaving(true);
    try {
      const { response, result } = await callAdminAction("send-mia-outreach-queue-item", { id: item.id });
      if (!response.ok) {
        console.error("Mia approved outreach send error:", result);
        showMessage("error", result.error || "Mia could not send the prepared outreach email.");
        return;
      }
      setProspects(result.prospects || prospects);
      setFollowUpTasks(result.followUps || followUpTasks);
      ...
      showMessage("error", "Mia could not send the prepared outreach email.");
    } finally {
      setIsSaving(false);
    }
  }

  function renderMiaOutreachWorkspace() {
    const outreachCounts = safeMiaOutreachQueue.reduce((counts, item) => ({ ...counts, [item.status]: (counts[item.status] || 0) + 1 }), {});
    return <div className="mt-5 rounded-2xl border border-blue-200 bg-blue-50 p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.18em] text-blue-700">Rory to Mia handoff</p>
          <h4 className="mt-1 text-lg font-black">Mia Outreach Workspace</h4>
          <p className="mt-1 max-w-3xl text-sm font-semibold leading-relaxed text-slate-600">See exactly what Mia prepared, what is waiting for your approval, what Resend accepted, and what was safely blocked. A prospect is only marked contacted after an email send is confirmed.</p>
        </div>
  ...
  function applyEllisOperationsResult(result) {
    setEllisEmails(result.emails || []);
    setEllisTasks(result.tasks || []);
  ...
  async function loadEllisOperations({ quiet = false } = {}) {
    setEllisBusy("loading");
    try {
      const { response, result } = await callAdminAction("get-ellis-operations");
      if (!response.ok) {
        console.error("Ellis operations load error:", result);
        if (!quiet) showMessage("error", result.error || "Could not load Ellis Operations Centre.");
        return null;
      }
      applyEllisOperationsResult(result);
      if (!quiet) showMessage("success", "Ellis Operations Centre refreshed.");
      return result;
    } catch (error) {
      console.error("Ellis operations load error:", error);
      if (!quiet) showMessage("error", "Could not load Ellis Operations Centre.");
      return null;
    } finally {
      setEllisBusy("");
    }
  ...
  async function submitEllisEmail(event) {
    ...
    setEllisBusy("triage");
    try {
      const { response, result } = await callAdminAction("save-ellis-email", { email: ellisEmailForm, source: "manual_back_office" });
      if (!response.ok) {
        console.error("Ellis triage save error:", result);
        showMessage("error", result.error || "Could not triage email.");
        return;
      }
      applyEllisOperationsResult(result);
      setEllisEmailForm({ sender_name: "", sender_email: "", subject: "", received_at: "", raw_excerpt: "" });
      showMessage("success", "Email analysed and added to Ellis Review Queue.");
    } catch (error) {
      console.error("Ellis triage save error:", error);
      showMessage("error", "Could not triage email.");
    } finally {
      setEllisBusy("");
    }
  }

  async function updateEllisEmail(email, updates, successMessage) {
    setEllisBusy(email.id);
    try {
      const { response, result } = await callAdminAction("update-ellis-email", { id: email.id, ...updates });
      if (!response.ok) {
        console.error("Ellis email update error:", result);
        showMessage("error", result.error || "Could not update email review.");
        return;
      }
      applyEllisOperationsResult(result);
      showMessage("success", successMessage || "Email review updated.");
    } catch (error) {
      console.error("Ellis email update error:", error);
      showMessage("error", "Could not update email review.");
    } finally {
      setEllisBusy("");
    }
  }

  async function createEllisFollowUp(email) {
    const due = new Date();
    ...
    try {
      ...
      if (!response.ok) {
        console.error("Ellis follow-up task error:", result);
        showMessage("error", result.error || "Could not create follow-up task.");
        return;
      }
      applyEllisOperationsResult(result);
      showMessage("success", "Follow-up task created.");
    } catch (error) {
  ...
  async function updateEllisTask(task, status) {
    setEllisBusy(task.id);
    try {
      const { response, result } = await callAdminAction("update-ellis-task", { id: task.id, status });
      if (!response.ok) {
        console.error("Ellis task update error:", result);
        showMessage("error", result.error || "Could not update task.");
        return;
      }
      applyEllisOperationsResult(result);
      showMessage("success", `Task marked ${status}.`);
    } catch (error) {
  ...
  async function generateEllisBriefing() {
    setEllisBusy("briefing");
    try {
      const { response, result } = await callAdminAction("generate-ellis-briefing");
      if (!response.ok) {
        console.error("Ellis briefing error:", result);
        showMessage("error", result.error || "Could not generate Ellis briefing.");
        return;
      }
      applyEllisOperationsResult(result);
      showMessage("success", "Ellis Daily Briefing generated.");
    } catch (error) {
      console.error("Ellis briefing error:", error);
      showMessage("error", "Could not generate Ellis briefing.");
    } finally {
      setEllisBusy("");
    }
  }
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-6 grid gap-4 lg:mt-8 lg:grid-cols-[240px_1fr] lg:gap-6">
          ...
          <section className="min-w-0 overflow-hidden rounded-2xl bg-white p-4 text-slate-950 shadow-sm sm:p-6">
            ...
            {activeTab === "Ellis Operations Centre" ? (
              <SafeSectionBoundary title="Ellis Operations Centre">
                <div>
                  <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div>
                      <p className="text-sm font-black uppercase tracking-[0.18em] text-emerald-700">Executive Operations Assistant</p>
                      <h2 className="mt-2 text-2xl font-black sm:text-3xl">Ellis Operations Centre</h2>
                  ...
                  <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-sm font-semibold text-slate-600">{safeText(latestEllisBriefing?.summary, "No briefing has been generated yet.")}</p>
                      </div>
                      <p className="text-xs font-black text-slate-500">{latestEllisBriefing?.created_at ? formatDisplayDateTime(latestEllisBriefing.created_at) : "No briefing yet"}</p>
                    </div>
                    <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
                      {[
                        ["Emails processed", ellisMetrics.total_emails_processed || 0, "bg-slate-50 text-slate-800"],
                        ["High priority", ellisMetrics.high_priority_count || 0, "bg-red-50 text-red-800"],
                        ["Customer enquiries", ellisMetrics.customer_enquiries || 0, "bg-emerald-50 text-emerald-800"],
                        ["Council emails", ellisMetrics.council_emails || 0, "bg-blue-50 text-blue-800"],
                        ["School emails", ellisMetrics.school_emails || 0, "bg-indigo-50 text-indigo-800"],
                        ["Follow-ups required", ellisMetrics.follow_ups_required || 0, "bg-amber-50 text-amber-800"],
                        ["Invoice / payment", ellisMetrics.invoice_payment_emails || 0, "bg-violet-50 text-violet-800"],
                        ["Marketing", ellisMetrics.marketing_emails || 0, "bg-cyan-50 text-cyan-800"],
                        ["Likely spam", ellisMetrics.likely_spam_count || 0, "bg-slate-100 text-slate-700"],
                    ...
                    <div className="mt-5 grid gap-4 lg:grid-cols-2">
                      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <h4 className="font-black">Top Urgent Emails</h4>
                        <div className="mt-3 grid gap-2">
                          {asArray(latestEllisBriefing?.urgent_emails).length ? asArray(latestEllisBriefing.urgent_emails).map((email, index) => <div key={email.id || `${email.subject}-${index}`} className="rounded-lg bg-white p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><p className="font-black">{safeText(email.sender, "Unknown sender")}</p><span className="rounded-full bg-red-100 px-2 py-1 text-xs font-black text-red-800">{safeText(email.priority, "Pending")}</span></div><p className="mt-1 font-bold text-slate-700">{safeText(email.subject, "No subject")}</p><p className="mt-1 text-slate-600">{safeText(email.summary, "No summary available")}</p></div>) : <p className="text-sm font-semibold text-slate-600">No urgent emails identified yet.</p>}
                        </div>
                      </div>
                      <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                        <h4 className="font-black">Today&apos;s Recommendations</h4>
                        <div className="mt-3 grid gap-2">
                          {asArray(latestEllisBriefing?.recommendations || ellisRecommendations).length ? asArray(latestEllisBriefing?.recommendations || ellisRecommendations).map((item, index) => <p key={`${item}-${index}`} className="rounded-lg bg-white p-3 text-sm font-semibold text-slate-700">{item}</p>) : <p className="text-sm font-semibold text-slate-600">No recommendations yet.</p>}
                        </div>
                      </div>
                    </div>
                  </section>
                  ...
                  <section className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                    ...
                    <form onSubmit={submitEllisEmail} className="mt-5 grid gap-4">
                      <div className="grid gap-4 md:grid-cols-2">
                        <input value={ellisEmailForm.sender_name} onChange={(e) => setEllisEmailForm((current) => ({ ...current, sender_name: e.target.value }))} className="rounded-xl border border-slate-200 bg-white p-3" placeholder="Sender name" />
                        <input type="email" value={ellisEmailForm.sender_email} onChange={(e) => setEllisEmailForm((current) => ({ ...current, sender_email: e.target.value }))} className="rounded-xl border border-slate-200 bg-white p-3" placeholder="Sender email" required />
                      </div>
                      <div className="grid gap-4 md:grid-cols-[1fr_220px]">
                        <input value={ellisEmailForm.subject} onChange={(e) => setEllisEmailForm((current) => ({ ...current, subject: e.target.value }))} className="rounded-xl border border-slate-200 bg-white p-3" placeholder="Email subject" required />
                        <input type="datetime-local" value={ellisEmailForm.received_at} onChange={(e) => setEllisEmailForm((current) => ({ ...current, received_at: e.target.value }))} className="rounded-xl border border-slate-200 bg-white p-3" />
                      </div>
                      <textarea value={ellisEmailForm.raw_excerpt} onChange={(e) => setEllisEmailForm((current) => ({ ...current, raw_excerpt: e.target.value }))} className="rounded-xl border border-slate-200 bg-white p-3" rows={5} placeholder="Paste the email body or useful excerpt" required />
                      <button type="submit" disabled={ellisBusy === "triage"} className="rounded-xl bg-slate-950 px-5 py-3 text-sm font-black text-white disabled:opacity-60">{ellisBusy === "triage" ? "Analysing..." : "Analyse and Add to Review Queue"}</button>
                    </form>
                  </section>
                  ...
                  <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-sm font-semibold text-slate-600">Every sensitive or uncertain email remains under your control.</p>
                      </div>
                      <div className="grid gap-2 sm:grid-cols-3">
                        <input value={ellisSearch} onChange={(e) => setEllisSearch(e.target.value)} className="rounded-xl border border-slate-200 p-3 text-sm" placeholder="Search emails" />
                        <select value={ellisCategoryFilter} onChange={(e) => setEllisCategoryFilter(e.target.value)} className="rounded-xl border border-slate-200 p-3 text-sm"><option value="">All categories</option>{ellisCategoryOptions.map((category) => <option key={category}>{category}</option>)}</select>
                        <select value={ellisPriorityFilter} onChange={(e) => setEllisPriorityFilter(e.target.value)} className="rounded-xl border border-slate-200 p-3 text-sm"><option value="">All priorities</option>{ellisPriorityOptions.map((priority) => <option key={priority}>{priority}</option>)}</select>
                      </div>
                    </div>
                    <div className="mt-5 grid gap-4">
                      {ellisReviewQueue.length ? ellisReviewQueue.map((email) => <article key={email.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                        <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                          <div>
                            <div className="flex flex-wrap gap-2">
                              <span className="rounded-full bg-slate-950 px-3 py-1 text-xs font-black text-white">{safeText(email.category, "Review Later")}</span>
                              <span className={`rounded-full px-3 py-1 text-xs font-black ${email.priority === "Critical" ? "bg-red-200 text-red-900" : email.priority === "High" ? "bg-amber-200 text-amber-900" : "bg-slate-200 text-slate-800"}`}>{safeText(email.priority, "Medium")}</span>
                              <span className="rounded-full bg-white px-3 py-1 text-xs font-black text-slate-600">Confidence {Number(email.confidence_score || 0)}%</span>
                            </div>
                            <h4 className="mt-3 text-lg font-black">{safeText(email.subject, "No subject")}</h4>
                            <p className="mt-1 text-sm font-bold text-slate-700">{safeText(email.sender_name, "Unknown sender")} · {safeText(email.sender_email, "No email")}</p>
                            <p className="mt-3 text-sm leading-relaxed text-slate-600">{safeText(email.summary, "No summary available")}</p>
                            <p className="mt-3 text-xs font-black text-emerald-700">Recommended action: {safeText(email.recommended_action, "Review")}</p>
                          </div>
                          <p className="text-xs font-bold text-slate-500">{formatDisplayDateTime(email.received_at)}</p>
                        </div>
                        <div className="mt-4 flex flex-wrap gap-2">
                          <button type="button" onClick={() => updateEllisEmail(email, { review_status: "Reviewed", requires_review: false }, "Email marked reviewed.")} disabled={ellisBusy === email.id} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-black text-white disabled:opacity-60">Mark Reviewed</button>
                          <button type="button" onClick={() => createEllisFollowUp(email)} disabled={ellisBusy === email.id} className="rounded-lg bg-blue-700 px-3 py-2 text-xs font-black text-white disabled:opacity-60">Create Follow-Up</button>
                          <button type="button" onClick={() => updateEllisEmail(email, { review_status: "Draft Requested", recommended_action: "Draft Reply", requires_review: true }, "Draft reply requested for manual review.")} disabled={ellisBusy === email.id} className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-xs font-black text-slate-700 disabled:opacity-60">Draft Reply</button>
                  ...
                  <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <h3 className="text-xl font-black">Follow-Up Tasks</h3>
                    <div className="mt-5 grid gap-3">
                      {safeEllisTasks.length ? safeEllisTasks.map((task) => <article key={task.id} className="grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 lg:grid-cols-[1fr_auto] lg:items-center"><div><div className="flex flex-wrap gap-2"><span className="rounded-full bg-blue-100 px-3 py-1 text-xs font-black text-blue-800">{safeText(task.status, "Open")}</span><span className="rounded-full bg-white px-3 py-1 text-xs font-black text-slate-700">{safeText(task.priority, "Medium")}</span></div><h4 className="mt-3 font-black">{safeText(task.title, "Untitled follow-up")}</h4><p className="mt-1 text-sm text-slate-600">{safeText(task.description, "No description")}</p><p className="mt-2 text-xs font-bold text-slate-500">Due: {task.due_date ? formatDisplayDate(task.due_date) : "Not set"} · Assigned: {safeText(task.assigned_user, "Admin")}</p></div><select value={task.status || "Open"} onChange={(e) => updateEllisTask(task, e.target.value)} disabled={ellisBusy === task.id} className="rounded-xl border border-slate-200 bg-white p-3 text-sm font-bold disabled:opacity-60"><option>Open</option><option>In Progress</option><option>Waiting</option><option>Completed</option></select></article>) : <p className="rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-600">No follow-up tasks yet.</p>}
                    </div>
                  </section>
                  ...
                  <section className="mt-6 grid gap-6 lg:grid-cols-2">
                    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                      ...
                      <p className="mt-2 text-sm font-semibold leading-relaxed text-slate-600">Fasthosts Livemail uses a read-only IMAP sync. Credentials stay inside Supabase Edge Function secrets and are never exposed to the browser.</p>
                      <div className="mt-4 grid gap-3">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                      <h3 className="text-xl font-black">Ellis Activity</h3>
                      <div className="mt-4 grid gap-2">
                  ...
                  <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
                      ...
                      <div className="grid gap-2 sm:grid-cols-4">
                        ...
                        <select value={opportunityFilters.status} onChange={(event) => setOpportunityFilters((current) => ({ ...current, status: event.target.value }))} className="rounded-xl border border-slate-200 p-3 text-sm"><option value="">Active statuses</option><option value="open">open</option><option value="won">won</option><option value="lost">lost</option><option value="dormant">archived / dormant</option></select>
              ...
              <SafeSectionBoundary title="Mia Communications">
                <div>
                  ...
                  <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                    ...
                    <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-[minmax(260px,1.4fr)_minmax(150px,0.8fr)_minmax(180px,0.9fr)_minmax(220px,1fr)]">
                      <label className="grid min-w-0 gap-1 text-xs font-black uppercase text-slate-600">Approval level<select value={Number(miaCommunicationSettings.approval_level || 1)} onChange={(event) => saveMiaCommunicationSettings({ approval_level: Number(event.target.value) })} disabled={miaCommunicationBusy === "settings"} className="min-w-0 w-full rounded-xl border border-slate-200 bg-white p-3 text-sm normal-case text-slate-900"><option value="1">Level 1: Draft only</option><option value="2">Level 2: One-click approval</option><option value="3">Level 3: Approved categories</option><option value="4">Level 4: Approved sequences</option></select></label>
