...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  const [aiRules, setAiRules] = useState({
    compliance: { approvalRequired: true, allowedActions: "Summarise compliance risks, prepare renewal task lists, draft reminder copy", forbiddenActions: "Delete records, change expiry dates, mark training as complete", escalationTriggers: "Expired training, missing certificates, failed reminders" },
    booking: { approvalRequired: true, allowedActions: "Draft booking summaries, prepare date options, flag missing details", forbiddenActions: "Confirm training dates automatically, send final booking confirmations without approval", escalationTriggers: "Date clash, payment issue, accessibility request, high delegate count" },
    outreach: { approvalRequired: true, allowedActions: "Draft outreach lists, prepare follow-up copy, suggest campaign segments", forbiddenActions: "Auto-send live outreach emails, scrape contacts, change CRM records", escalationTriggers: "Complaint risk, unsubscribe request, public-sector procurement enquiry" },
    content: { approvalRequired: false, allowedActions: "Draft blog ideas, create content outlines, summarise training updates", forbiddenActions: "Publish public pages without review, change pricing, alter legal copy", escalationTriggers: "Legal wording, safeguarding wording, council-facing claims" },
    email: { approvalRequired: true, allowedActions: "Draft replies, summarise inbox themes, prepare reminder wording", forbiddenActions: "Auto-send real emails, attach private files, confirm bookings", escalationTriggers: "Complaint, safeguarding concern, payment dispute, booking confirmation" }
  });
  ...
  const aiAgentCards = [
    { key: "compliance", name: "Compliance Agent", status: "Simulation ready", actionsToday: expiredTrainingRecords.length + certificateMissingRecords.length, pendingTasks: expiringWithinThirtyDays.length, approvalsRequired: expiredTrainingRecords.length, lastActivity: activity[0] ? "Current session" : "No activity yet" },
    { key: "booking", name: "Booking Agent", status: "Approval locked", actionsToday: onboarding.length, pendingTasks: onboarding.filter((item) => (item.status || "New") !== "Completed").length, approvalsRequired: onboarding.length, lastActivity: "Awaiting human date approval" },
    { key: "outreach", name: "Outreach Agent", status: "Simulation only", actionsToday: 0, pendingTasks: reviews.filter((review) => review.status === "Draft").length, approvalsRequired: 1, lastActivity: "Campaign drafts only" },
    { key: "content", name: "Content Agent", status: "Drafting enabled", actionsToday: posts.length, pendingTasks: posts.filter((post) => post.status === "Draft").length, approvalsRequired: posts.filter((post) => post.status === "Draft").length, lastActivity: posts[0]?.title || "No content activity" },
    { key: "email", name: "Email Agent", status: "Simulation only", actionsToday: trainingReminderLogs.length, pendingTasks: trainingReminders.filter((reminder) => reminder.status === "pending").length, approvalsRequired: trainingReminders.filter((reminder) => reminder.status === "failed").length, lastActivity: trainingReminderLogs[0]?.created_at ? formatDisplayDateTime(trainingReminderLogs[0].created_at) : "No reminder emails yet" }
  ];
  const aiApprovalQueue = [
    { id: "booking-date-approval", agent: "Booking Agent", task: "Review preferred training dates before sending final confirmation", risk: "Booking Agent cannot confirm dates automatically", count: onboarding.length },
    { id: "expired-training-followup", agent: "Compliance Agent", task: "Approve renewal action plan for expired training records", risk: "Expired training requires human review", count: expiredTrainingRecords.length },
    { id: "failed-reminder-review", agent: "Email Agent", task: "Review failed reminder notifications before retry", risk: "Failed emails may need manual contact", count: reminderFailureRecords.length }
          ...
          <section className="min-w-0 overflow-hidden rounded-2xl bg-white p-4 text-slate-950 shadow-sm sm:p-6">
            ...
            {activeTab === "AI Operations" ? (
              <div>
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  <div>
                    <p className="text-sm font-black uppercase tracking-[0.18em] text-emerald-700">Simulation mode active</p>
                    <h2 className="mt-2 text-2xl font-black sm:text-3xl">AI Operations Centre</h2>
                    <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-600">Future AI-assisted workflows can be reviewed here. No real emails are sent and no bookings are confirmed automatically.</p>
                  </div>
                  <span className="rounded-xl bg-amber-50 px-4 py-3 text-sm font-black text-amber-800">Safe testing mode</span>
                </div>
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                  ...
                  <p className="mt-3 text-sm leading-relaxed text-slate-700">Today&apos;s simulated summary: {expiredTrainingRecords.length} expired training record(s), {expiringWithinSevenDays.length} record(s) expiring within 7 days, {certificateMissingRecords.length} missing certificate(s), and {reminderFailureRecords.length} reminder failure(s) need review.</p>
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="mt-5 grid gap-3">
                    {aiApprovalQueue.length ? aiApprovalQueue.map((item) => <div key={item.id} className="grid gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 lg:grid-cols-[1fr_auto_auto] lg:items-center"><div><p className="font-black">{item.agent}</p><p className="mt-1 text-sm font-semibold text-slate-700">{item.task}</p><p className="mt-1 text-xs font-bold text-amber-700">{item.risk} - {item.count} item(s)</p></div><button type="button" onClick={() => simulateAiApproval(item, "Approved")} className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-black text-white">Approve Simulation</button><button type="button" onClick={() => simulateAiApproval(item, "Dismissed")} className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-black text-slate-700">Dismiss Simulation</button></div>) : <p className="rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-600">No approval items are waiting in simulation mode.</p>}
                  </div>
                </section>
