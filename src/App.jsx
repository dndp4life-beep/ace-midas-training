...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  const enrichedTrainingRecords = tcRecords.map((record) => {
    const organisation = tcOrganisationMap[record.organisation_id];
    ...
    return { ...record, organisation, staffMember, course, expiry_date: expiryDate, status: normaliseStatus(record.status || getTrainingStatus(expiryDate)) };
  });
  const filteredTrainingRecords = enrichedTrainingRecords.filter((record) => {
    if (tcFilters.organisation_id && record.organisation_id !== tcFilters.organisation_id) return false;
  ...
  useEffect(() => {
    ...
    async function loadTrainingComplianceData() {
      const [organisationsResult, membersResult, coursesResult, recordsResult] = await Promise.all([
        ...
        supabase.from("training_records").select("id, organisation_id, member_id, course_id, date_completed, expiry_date, status, created_at").order("expiry_date", { ascending: true })
      ]);
  ...
  async function saveTrainingOrganisation(e) {
    ...
    const { data, error } = await supabase.from("organisations").insert(organisation).select("id, name, contact_name, contact_email, phone, created_at").single();
    if (error) {
      showMessage("error", "Could not save changes. Please try again.");
      return;
    }
    setTcOrganisations((current) => [...current, data].sort((a, b) => String(a.name).localeCompare(String(b.name))));
  ...
  async function saveTrainingOrganisationRow(org) {
    ...
    showMessage(error ? "error" : "success", error ? "Could not save changes. Please try again." : "Organisation saved successfully");
  }
  ...
  async function saveTrainingMember(e) {
    ...
    const { data, error } = await supabase.from("members").insert(member).select("id, organisation_id, full_name, email, role, created_at").single();
    if (error) {
      showMessage("error", "Could not save changes. Please try again.");
      return;
    }
    setTcMembers((current) => [...current, data].sort((a, b) => String(a.full_name).localeCompare(String(b.full_name))));
  ...
  async function saveTrainingMemberRow(member) {
    ...
    showMessage(error ? "error" : "success", error ? "Could not save changes. Please try again." : "Staff member saved successfully");
  }
  ...
  async function saveTrainingRecord(e) {
    ...
    const record = {
      organisation_id: tcRecordForm.organisation_id,
    ...
    if (!record.organisation_id || !record.member_id || !record.course_id || !record.date_completed) {
    ...
    const { data, error } = await supabase.from("training_records").insert(record).select("id, organisation_id, member_id, course_id, date_completed, expiry_date, status, created_at").single();
    if (error) {
      showMessage("error", "Could not save changes. Please try again.");
      return;
    }
  ...
  useEffect(() => {
    async function loadSiteSettings() {
      ...
      const { data, error } = await supabase
        ...
        .maybeSingle();
      if (!error && data?.setting_value) setSiteSettings((current) => ({ ...current, ...data.setting_value }));
    }
    loadSiteSettings();
  }, []);
