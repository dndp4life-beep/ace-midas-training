...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  function buildReportStatus(report) {
    ...
    return {
      ...
      status: report.type === "council" ? "Ready to generate PDF" : report.action === "pdf" ? "PDF generation is not active yet" : report.action === "stub" ? "Report data source is being connected" : "Ready to export",
      description: report.description,
      counts: {
        organisations: tcOrganisations.length,
        staff: tcMembers.length,
        records: report.type === "organisation" ? tcOrganisations.length : records.length,
        valid: trainingSummary.valid,
        expiring7: expiringWithinSevenDays.length,
        expiring30: expiringWithinThirtyDays.length,
        expired: expiredTrainingRecords.length,
        missing: missingTrainingMembers.length
      },
      updatedAt: new Date().toISOString()
    };
  }

  function generateCouncilCompliancePdf() {
    const doc = new jsPDF({ orientation: "landscape", unit: "mm", format: "a4" });
    const generatedAt = new Date().toISOString();
    const organisationName = tcOrganisations.length === 1 ? tcOrganisations[0].name : tcOrganisations.length > 1 ? "All Organisations" : "No Organisation Selected";
    ...
    const records = enrichedTrainingRecords;
    const compliantStaffCount = fullyCompliantStaff.length;
    const actionItems = [
      expiredTrainingRecords.length ? `${expiredTrainingRecords.length} expired training record(s) need immediate renewal action.` : "No expired training records currently recorded.",
      expiringWithinSevenDays.length ? `${expiringWithinSevenDays.length} record(s) expire within 7 days and should be prioritised.` : "No records expire within the next 7 days.",
      expiringWithinThirtyDays.length ? `${expiringWithinThirtyDays.length} record(s) expire within 30 days and should be scheduled for renewal.` : "No records expire within the next 30 days.",
      missingTrainingMembers.length ? `${missingTrainingMembers.length} staff member(s) have no training record logged.` : "All listed staff have at least one training record.",
    ...
    doc.setFontSize(12);
    doc.text("Council Compliance Report", 14, 22);

    doc.setTextColor(15, 23, 42);
    doc.setFontSize(18);
    doc.text("Council Compliance Report", 14, 43);
    ...
    autoTable(doc, {
      ...
      body: [[tcMembers.length, compliantStaffCount, expiredTrainingRecords.length, expiringWithinSevenDays.length, expiringWithinThirtyDays.length, missingTrainingMembers.length]],
    ...
    autoTable(doc, {
      ...
      theme: "striped",
      head: [["Organisation", "Staff member", "Email", "Role", "Course", "Completed", "Expiry", "Status"]],
  ...
  async function handleReportAction(report) {
    ...
    try {
      ...
      if (report.type === "council") {
        const fileName = generateCouncilCompliancePdf();
        setReportStatusPanel((current) => current ? { ...current, status: "PDF generated", fileName } : current);
        showMessage("success", "Council Compliance Report PDF downloaded.");
        return;
      }
  ...
  const premiumReportCards = [
    { title: "Council Compliance Report", type: "council", format: "PDF", action: "pdf", description: "Council-ready overview of staff training, expiry status and provider compliance evidence." },
                    ...
                    {premiumReportCards.map((report) => (
                      <article key={report.title} className="flex min-h-[220px] flex-col justify-between rounded-2xl border border-white/10 bg-white p-5 text-slate-950 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-xl">
                        ...
                        <button type="button" onClick={() => handleReportAction(report)} disabled={generatingReport === report.title} className="mt-6 rounded-xl bg-slate-950 px-4 py-3 text-sm font-black text-white disabled:cursor-wait disabled:opacity-60">
                          {generatingReport === report.title ? "Working..." : report.action === "pdf" ? "Generate PDF" : report.action === "stub" ? "View Status" : "Generate / Export"}
                        </button>
                      </article>
                    ))}
                  </div>
