...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  const aiAgentCards = [
    ...
    { key: "theo", role: "Booking Agent", status: "Approval required", actionsToday: agentLogsTodayFor("theo").length, pendingTasks: onboarding.filter((item) => (item.status || "New") !== "Completed").length, approvalsRequired: onboarding.length, lastActivity: agentLogsFor("theo")[0]?.created_at ? formatDisplayDateTime(agentLogsFor("theo")[0].created_at) : "Awaiting human date approval" },
