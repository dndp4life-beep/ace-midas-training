...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  const [aiRules, setAiRules] = useState({
    ...
    theo: { approvalRequired: false, allowedActions: "Answer safe booking process questions, explain group or onsite training in general terms, gather attendee numbers, course type, location and preferred timeframe", forbiddenActions: "Confirm training dates, promise availability, change bookings, cancel bookings, agree custom prices, offer non-standard discounts, send/request custom Stripe links", escalationTriggers: "Exact dates, confirmed availability, booking changes, cancellations, pricing exceptions, discounts, payment links" },
  ...
  const tabs = ["Dashboard", "Blogs", "Reviews", "Onboarding", "Members", "Training Compliance", "Reports & Exports", "Export Centre", "Depot Tokens", "Activity", "AI Operations", "Media Manager", "Settings"];
  ...
  const theoResponsesSentToday = agentLogsTodayFor("theo").filter((log) => log.action_type === "theo_response_sent").length;
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="mt-5 overflow-x-auto">
                    <table className="min-w-[900px] w-full text-left text-sm">
                      <thead className="bg-slate-100 text-xs uppercase text-slate-500"><tr><th className="p-3">Received</th><th className="p-3">Sender</th><th className="p-3">Subject</th><th className="p-3">Classification</th><th className="p-3">Agent</th><th className="p-3">Status</th><th className="p-3">Action</th></tr></thead>
                      <tbody>{inboundMessages.length ? inboundMessages.map((item) => <tr key={item.id} className="border-t border-slate-200"><td className="p-3">{formatDisplayDateTime(item.created_at)}</td><td className="p-3">{item.from_name || item.from_email || "Unknown"}<p className="text-xs text-slate-500">{item.organisation}</p></td><td className="p-3">{item.subject || "No subject"}</td><td className="p-3 font-bold">{item.classification}</td><td className="p-3 font-bold uppercase">{item.assigned_agent}</td><td className="p-3">{item.status}</td><td className="p-3">{item.action_taken}</td></tr>) : <tr><td className="p-4 text-sm font-bold text-slate-600" colSpan={7}>No inbound messages captured yet.</td></tr>}</tbody>
                    </table>
                  </div>
                </section>
