...
const demoMembers = [
  {
    ...
    access: { depotSiteLogin: "https://journeytracker.manus.space/login?token=demoabc123xyz789" },
  ...
  return (
    <main className="overflow-hidden">
      <section className="relative min-h-[88vh] overflow-hidden bg-slate-950 text-white">
        <div className="absolute inset-0"><img src={images.vehicleLineup} alt="Passenger transport fleet" className="h-full w-full object-cover opacity-35" /><div className="absolute inset-0 bg-gradient-to-r from-slate-950 via-slate-950/90 to-slate-950/30" /></div>
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-6 py-20 lg:grid-cols-[1.05fr_0.95fr] lg:py-28">
          ...
          <div className="relative hidden lg:block"><div className="relative rotate-2 overflow-hidden rounded-[2.5rem] border border-white/10 bg-white/10 p-4 shadow-2xl backdrop-blur transition hover:rotate-0"><img src={images.minibusHero} alt="ACE MiDAS minibus training" className="h-[520px] w-full rounded-[2rem] object-cover" /><div className="absolute bottom-8 left-8 right-8 rounded-3xl border border-white/10 bg-slate-950/85 p-6 shadow-xl backdrop-blur"><p className="text-sm font-semibold text-emerald-300">ACE MiDAS Training</p><p className="mt-1 text-3xl font-black">Training that matches real transport operations.</p><p className="mt-3 text-sm leading-6 text-slate-300">Built around the daily realities of SEND, school and passenger transport services.</p></div></div></div>
        </div>
      </section>
      ...
      <section className="bg-slate-100 px-6 py-20"><div className="mx-auto max-w-7xl"><div className="mx-auto max-w-3xl text-center"><p className="font-semibold text-emerald-700">How it works</p><h2 className="mt-3 text-4xl font-black tracking-tight md:text-6xl">A simple journey from booking to evidence.</h2></div><div className="mt-12 grid gap-6 md:grid-cols-3">{steps.map((step) => <div key={step.title} className="rounded-[2rem] border border-slate-200 bg-white p-7 shadow-sm"><h3 className="text-2xl font-black">{step.title}</h3><p className="mt-3 leading-7 text-slate-600">{step.text}</p></div>)}</div></div></section>
...
function LoginPage({ setPage, setLoggedInMember }) {
  ...
  function handleCredentials(e) { e.preventDefault(); setError(""); if (!email.includes("@") || password.length < 6) { setError("Please enter a valid email address and a password of at least 6 characters."); return; } const matched = demoMembers.find((item) => item.email.toLowerCase() === email.toLowerCase()); const fallback = { email, organisation: "Demo Member Organisation", role: "Depot / Site Manager", access: { depotSiteLogin: "https://journeytracker.manus.space/login?token=demoabc123xyz789" }, onboardingStatus: "Setup required" }; setPendingMember(matched || fallback); setStep("verification"); }
...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews }) { const [code, setCode] = useState(""); const [unlocked, setUnlocked] = useState(false); const [newPost, setNewPost] = useState({ tag: "Industry Update", title: "", text: "", status: "Draft" }); if (!unlocked) return <main className="min-h-screen bg-slate-950 px-6 py-20 text-white"><form onSubmit={(e) => { e.preventDefault(); if (code === "ACEADMIN2026") setUnlocked(true); }} className="mx-auto max-w-xl rounded-3xl bg-white p-8 text-slate-950"><h1 className="text-3xl font-black">Admin Login</h1><input value={code} onChange={(e) => setCode(e.target.value)} className="mt-6 w-full rounded-xl border p-4" placeholder="Back office code" type="password" /><button type="submit" className="mt-4 w-full rounded-xl bg-slate-950 p-4 font-black text-white">Enter</button><button type="button" onClick={() => setPage("Home")} className="mt-3 w-full rounded-xl border p-4 font-bold">Back</button></form></main>; return <main className="min-h-screen bg-slate-950 px-6 py-20 text-white"><div className="mx-auto max-w-7xl"><h1 className="text-5xl font-black">ACE Back Office</h1><div className="mt-10 grid gap-8 lg:grid-cols-2"><section className="rounded-3xl bg-white p-7 text-slate-950"><h2 className="text-3xl font-black">Add Blog Update</h2><form onSubmit={(e) => { e.preventDefault(); if (newPost.title) setPosts([newPost, ...posts]); }} className="mt-6 grid gap-4"><input value={newPost.tag} onChange={(e) => setNewPost({ ...newPost, tag: e.target.value })} className="rounded-xl border p-3" /><input value={newPost.title} onChange={(e) => setNewPost({ ...newPost, title: e.target.value })} className="rounded-xl border p-3" placeholder="Blog title" /><textarea value={newPost.text} onChange={(e) => setNewPost({ ...newPost, text: e.target.value })} className="rounded-xl border p-3" rows={5} placeholder="Write update" /><select value={newPost.status} onChange={(e) => setNewPost({ ...newPost, status: e.target.value })} className="rounded-xl border p-3"><option>Draft</option><option>Published</option></select><button className="rounded-xl bg-emerald-600 p-4 font-black text-white">Save Blog</button></form></section><section className="rounded-3xl bg-white p-7 text-slate-950"><h2 className="text-3xl font-black">Stats</h2><p className="mt-4">Published blogs: {posts.filter((p) => p.status === "Published").length}</p><p>Published reviews: {reviews.filter((r) => r.status === "Published").length}</p></section></div></div></main>; }
