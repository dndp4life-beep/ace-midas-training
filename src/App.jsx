...
const BACK_OFFICE_UNLOCK_STORAGE_KEY = "ace_back_office_unlocked";
...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  const tabs = ["Dashboard", "Blogs", "Reviews", "Onboarding", "Members", "Training Compliance", "Reports & Exports", "Export Centre", "Depot Tokens", "Activity", "AI Operations", "Ava Compliance Centre", "Nia Content Studio", "Rory Prospecting Centre", "Workflow Debug Trace", "Media Manager", "Settings"];
  ...
  async function sendQualifiedProspectsToMia() {
    const prospectsToSend = qualifiedProspectsForMia.slice(0, 10);
    if (!prospectsToSend.length) {
      showMessage("error", "No qualified high-priority prospects are ready for Mia.");
      return;
    }
    setIsSaving(true);
    let sent = 0;
  ...
  useEffect(() => {
    try {
      const savedUnlocked = window.localStorage.getItem(BACK_OFFICE_UNLOCK_STORAGE_KEY);
      ...
      console.warn("Back Office session restore unavailable:", error);
    }
  }, []);
  ...
  useEffect(() => {
    if (!unlocked) return;
    try {
      window.localStorage.setItem(BACK_OFFICE_UNLOCK_STORAGE_KEY, "true");
  ...
  useEffect(() => {
    if (!unlocked || !["Training Compliance", "Export Centre", "Reports & Exports", "Rory Prospecting Centre"].includes(activeTab)) return;
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    ...
                    <div className="mt-4 grid gap-3 lg:grid-cols-[1fr_0.8fr_auto_auto]">
                      ...
                      <button type="button" onClick={sendQualifiedProspectsToMia} disabled={isSaving || !qualifiedProspectsForMia.length} className="rounded-xl border border-blue-300 bg-white px-5 py-3 text-sm font-black text-blue-800 disabled:opacity-50">Send Qualified Prospects to Mia</button>
                    </div>
                  ...
                  <div className="mt-5 grid gap-3">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                      <div>
                        ...
                        <p className="mt-1 text-sm font-semibold text-slate-600">High-priority prospects can be passed to Mia for a warm outreach draft or automated send when that setting is enabled.</p>
                      </div>
                      <p className="rounded-xl bg-blue-50 px-4 py-3 text-sm font-black text-blue-800">{safeProspects.length} on record | {highPriorityProspects.length} high priority | {contactedProspects.length} contacted</p>
                    </div>
                    {safeProspects.length ? safeProspects.map((prospect) => <article key={prospect.id || prospect.organisation_name} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                          ...
                          <p className="mt-2 text-xs font-bold text-slate-500">Rory scores from sector fit, recommended service fit, public contact quality, source completeness and relevant keywords. Imported research scores are preserved when they are higher than Rory's local calculation.</p>
