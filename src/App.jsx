...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings, loginNonce }) {
  ...
  const filteredProspects = safeProspects.filter(prospectMatchesServiceFilter);
  const filteredHighPriorityProspects = highPriorityProspects.filter(prospectMatchesServiceFilter);
  const filteredQualifiedProspectsForMia = qualifiedProspectsForMia.filter(prospectMatchesServiceFilter);
              ...
              <div>
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="mt-5 grid gap-3">
                    ...
                    {roryProspectServiceFilter ? <p className="rounded-xl bg-emerald-50 p-3 text-sm font-bold text-emerald-800">Showing prospects matched to {roryProspectServiceFilter}. Bulk sending will only use qualified high-priority prospects in this filtered view.</p> : null}
                    {filteredProspects.length ? filteredProspects.map((prospect) => <article key={prospect.id || prospect.organisation_name} className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                      <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                        <div>
                    ...
                    </article>) : <p className="rounded-xl bg-slate-50 p-4 text-sm font-bold text-slate-600">{roryProspectServiceFilter ? `No ${roryProspectServiceFilter} prospects match this filter yet.` : "No prospects have been added yet."}</p>}
                  </div>
                </section>
                </> : null}
