                      ...
                      <table className="min-w-[1120px] w-full text-left text-sm">
                        ...
                        <tbody>
                          {filteredTrainingRecords.map((record) => {
                            ...
                            return <tr key={record.id} className="border-t border-slate-200"><td className="p-3">{record.organisation?.name || "Unknown"}</td><td className="p-3 font-bold">{record.staffMember?.full_name || "Unknown"}</td><td className="p-3">{record.staffMember?.email || ""}</td><td className="p-3">{record.staffMember?.role || ""}</td><td className="p-3">{record.course?.name || "Unknown"}</td><td className="p-3">{formatDisplayDate(record.date_completed)}</td><td className="p-3">{formatDisplayDate(record.expiry_date)}</td><td className="p-3"><span className={`rounded-full px-3 py-1 text-xs font-black ${statusClass}`}>{record.status}</span></td><td className="p-3 font-bold">{recordEvidence.length ? "Yes" : "No"}</td><td className="p-3"><div className="flex flex-wrap gap-2"><input id={uploadInputId} type="file" accept="application/pdf,image/jpeg,image/png,image/webp" className="hidden" onChange={(e) => { uploadTrainingEvidence(record, e.target.files?.[0]); e.target.value = ""; }} /><button type="button" onClick={() => document.getElementById(uploadInputId)?.click()} disabled={evidenceBusyId === record.id} className="rounded-lg bg-emerald-600 px-3 py-2 text-xs font-black text-white disabled:opacity-60">Upload Evidence</button><button type="button" onClick={() => viewTrainingEvidence(record)} className="rounded-lg border border-slate-300 px-3 py-2 text-xs font-black text-slate-700">View Evidence</button><button type="button" onClick={() => downloadLatestTrainingEvidence(record)} disabled={!recordEvidence.length || evidenceBusyId === recordEvidence[0]?.id} className="rounded-lg bg-slate-950 px-3 py-2 text-xs font-black text-white disabled:opacity-40">Download Evidence</button><button type="button" onClick={() => deleteLatestTrainingEvidence(record)} disabled={!recordEvidence.length || evidenceBusyId === recordEvidence[0]?.id} className="rounded-lg bg-red-600 px-3 py-2 text-xs font-black text-white disabled:opacity-40">Delete Evidence</button><button type="button" onClick={() => deleteTrainingRecord(record)} className="rounded-lg bg-red-700 px-3 py-2 text-xs font-black text-white">Delete Record</button></div></td></tr>;
                          })}
                        </tbody>
                      </table>
                    </div>
