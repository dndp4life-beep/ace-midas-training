...
const ADMIN_SAVE_SETTINGS_API_URL = "/api/admin-save-settings";
const ADMIN_GET_SETTINGS_API_URL = "/api/admin-get-settings";
const ADMIN_SAVE_ORGANISATION_API_URL = "/api/admin-save-organisation";
...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  const tabs = ["Dashboard", "Blogs", "Reviews", "Onboarding", "Members", "Training Compliance", "Depot Tokens", "Activity", "Media Manager", "Settings"];
  ...
  useEffect(() => {
    ...
    async function loadTrainingComplianceData() {
      const [organisationsResult, membersResult, coursesResult, recordsResult] = await Promise.all([
        supabase.from("organisations").select("id, name, contact_name, contact_email, phone, created_at").order("name", { ascending: true }),
        supabase.from("members").select("id, organisation_id, full_name, email, role").order("full_name", { ascending: true }),
        supabase.from("courses").select("id, name, validity_months, created_at").order("name", { ascending: true }),
        supabase.from("training_records").select("id, member_id, course_id, date_completed, expiry_date, status").order("expiry_date", { ascending: true })
      ]);

      const loadError = organisationsResult.error || membersResult.error || coursesResult.error || recordsResult.error;
      if (loadError) {
        showMessage("error", loadError.message || "Could not load training compliance data.");
        return;
      }

      setTcOrganisations(organisationsResult.data || []);
      setTcMembers(membersResult.data || []);
      setTcCourses(coursesResult.data || []);
      setTcRecords(recordsResult.data || []);
      setActivity((current) => ["Loaded training compliance records", ...current]);
    }
  ...
  async function saveSiteSettings() {
    try {
      const response = await fetch(ADMIN_SAVE_SETTINGS_API_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const verifyResponse = await fetch(ADMIN_GET_SETTINGS_API_URL);
  ...
  async function refreshTrainingOrganisations() {
    if (!supabase) return [];
    const { data, error } = await supabase.from("organisations").select("id, name, contact_name, contact_email, phone, created_at").order("name", { ascending: true });
    if (error) {
      console.error("Organisation refresh error:", error);
      return [];
    }
    setTcOrganisations(data || []);
    setDebugInfo((current) => ({ ...current, lastFetchCount: data?.length || 0 }));
    return data || [];
  }
  ...
  async function saveTrainingOrganisation(e) {
    ...
    try {
      const response = await fetch(ADMIN_SAVE_ORGANISATION_API_URL, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function saveTrainingMember(e) {
    ...
    if (!member.organisation_id || !member.full_name) {
      showMessage("error", "Organisation and staff name are required.");
      return;
    }
    if (!supabase) {
      showMessage("error", "Could not save changes. Please try again.");
    ...
    const { data, error } = await supabase.from("members").insert(member).select("id, organisation_id, full_name, email, role").single();
    if (error) {
      console.error("Staff member save error:", error);
      showMessage("error", error.message || "Could not save changes. Please try again.");
      return;
    }
    const { data: refreshedMembers, error: refreshError } = await supabase.from("members").select("id, organisation_id, full_name, email, role").order("full_name", { ascending: true });
    if (refreshError) console.error("Staff members refresh error:", refreshError);
    setTcMembers(refreshedMembers || ((current) => [...current, data].sort((a, b) => String(a.full_name).localeCompare(String(b.full_name)))));
    setTcMemberForm({ organisation_id: "", full_name: "", email: "", role: "" });
    setActivity((current) => [`Staff member added: ${data.full_name}`, ...current]);
  ...
  async function saveTrainingMemberRow(member) {
    ...
    if (error) console.error("Staff member update error:", error);
    showMessage(error ? "error" : "success", error ? (error.message || "Could not save changes. Please try again.") : "Staff member saved successfully");
  }
  ...
  async function saveTrainingRecord(e) {
    ...
    const { data, error } = await supabase.from("training_records").insert(record).select("id, member_id, course_id, date_completed, expiry_date, status").single();
    if (error) {
      console.error("Training record save error:", error);
      showMessage("error", error.message || "Could not save changes. Please try again.");
      return;
    }
    setTcRecords((current) => [data, ...current]);
    setTcRecordForm({ organisation_id: "", member_id: "", course_id: "", date_completed: "" });
    setActivity((current) => [`Training record saved: ${tcMemberMap[data.member_id]?.full_name || "staff member"}`, ...current]);
                ...
                <div className="mt-8 grid gap-6">
                  <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                    ...
                    <div className="mt-5 grid gap-3">
                      {tcOrganisations.map((org) => <div key={org.id} className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 lg:grid-cols-[1.5fr_1fr_1fr_1fr_auto]">
                  ...
                  <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                    ...
                    <form onSubmit={saveTrainingMember} className="mt-4 grid gap-4 md:grid-cols-5">
                      ...
                      <input type="email" name="email" value={tcMemberForm.email} onChange={updateTcMemberForm} className="rounded-xl border border-slate-200 p-3" placeholder="Email" />
                      <input name="role" value={tcMemberForm.role} onChange={updateTcMemberForm} className="rounded-xl border border-slate-200 p-3" placeholder="Role" />
                      <button type="submit" className="rounded-xl bg-slate-950 p-3 font-black text-white">Add Staff</button>
                    </form>
                    <div className="mt-5 grid gap-3">
                      {tcMembers.map((member) => <div key={member.id} className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 lg:grid-cols-[1.2fr_1fr_1fr_1fr_auto]">
                  ...
                  <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                    ...
                    <div className="mt-5 overflow-x-auto rounded-xl border border-slate-200 bg-white">
                      <table className="min-w-[900px] w-full text-left text-sm">
                        <thead className="bg-slate-100 text-slate-600"><tr><th className="p-3">Organisation</th><th className="p-3">Member</th><th className="p-3">Email</th><th className="p-3">Role</th><th className="p-3">Course</th><th className="p-3">Completed</th><th className="p-3">Expiry</th><th className="p-3">Status</th></tr></thead>
                        <tbody>
...
export default function App() {
  ...
  useEffect(() => {
    async function loadSiteSettings() {
      const response = await fetch(ADMIN_GET_SETTINGS_API_URL);
      const text = await response.text();
      const result = text ? JSON.parse(text) : {};
      if (!response.ok) {
