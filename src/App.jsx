...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  async function updateReplyApproval(reply, approvalStatus) {
    setIsSaving(true);
    try {
      const { response, result } = await callAdminAction("update-reply-approval", { id: reply.id, approval_status: approvalStatus, notes: replyApprovalNotes[reply.id] || "" });
      if (!response.ok) {
            ...
            {activeTab === "AI Operations" ? (
              <div>
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                  ...
                  <p className="mt-3 text-sm leading-relaxed text-slate-700">Today&apos;s summary: Ava found {expiringWithinThirtyDays.length} record(s) expiring within 30 days. Mia sent {remindersSentToday} refresher reminder(s){miaWorkflowStats ? `, with ${miaWorkflowStats.skippedDuplicates || 0} duplicate(s) skipped and ${miaWorkflowStats.failedSends || 0} failed send(s) in the latest run` : ""}. Mia/Ellis classified {repliesClassifiedToday} customer reply/replies. Rory added or updated {prospectsAddedToday} prospect(s). Ellis filtered {emailsFilteredToday} email(s). Nia created {contentDraftsToday} content draft(s). Theo has {bookingApprovalsNeeded} booking approval item(s).</p>
                      ...
                      return <article key={reply.id} className="rounded-2xl border border-amber-200 bg-amber-50 p-4">
                        <div className="grid gap-3 lg:grid-cols-[1fr_auto] lg:items-start">
                          ...
                          <div className="grid gap-2">
                            <button type="button" onClick={() => updateReplyApproval(reply, "approved")} className="rounded-lg bg-emerald-600 px-4 py-2 text-sm font-black text-white">Approve</button>
                            <button type="button" onClick={() => updateReplyApproval(reply, "rejected")} className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-black text-slate-700">Reject</button>
                          </div>
                        </div>
                        <textarea value={replyApprovalNotes[reply.id] || ""} onChange={(e) => setReplyApprovalNotes((current) => ({ ...current, [reply.id]: e.target.value }))} className="mt-3 w-full rounded-xl border border-amber-200 bg-white p-3 text-sm" rows={2} placeholder="Theo approval notes" />
                      </article>;
