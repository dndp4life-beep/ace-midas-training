...
const trainingCourses = [
  { title: "MiDAS Standard", price: "Â£165", note: "Includes Â£40 CTA learner-pass charge", image: images.driverSeat, mediaSlot: "midas_standard" },
  { title: "MiDAS Accessible", price: "Â£210", note: "Includes Â£40 CTA learner-pass charge", image: images.driverSeat, mediaSlot: "midas_accessible" },
  { title: "PATS Standard", price: "Â£125", note: "Includes Â£30 CTA learner-pass charge", image: images.interior, mediaSlot: "pats_standard" },
  { title: "PATS Accessible", price: "Â£155â€“Â£185", note: "Attendance or proficiency routes", image: images.interior, mediaSlot: "pats_accessible" },
  { title: "First Aid at Work", price: "Â£205â€“Â£225", note: "Blended or 3-day classroom options", image: images.firstAid, mediaSlot: "first_aid_at_work" },
  { title: "Childrenâ€™s Transport First Aid", price: "Â£95â€“Â£135", note: "Optional epilepsy medication module", image: images.firstAid, mediaSlot: "children_transport_first_aid" }
];
...
const stats = [["MiDAS", "Driver training"], ["PATS", "Passenger assistant training"], ["CTFA", "Childrenâ€™s transport first aid"], ["Compliance", "Digital tracking support"]];
const initialPosts = [
...
const initialReviews = [
  { rating: "â˜…â˜…â˜…â˜…â˜…", name: "Transport Manager", org: "SEND Transport Provider", text: "The system gives us a clearer way to evidence what happens on each journey.", status: "Published" },
  { rating: "â˜…â˜…â˜…â˜…â˜…", name: "School Operations Lead", org: "Academy Trust", text: "The training and compliance approach feels practical, organised and relevant.", status: "Published" }
];
...
function runSelfTests() {
  ...
  console.assert(trainingCourses.some((course) => course.title === "Childrenâ€™s Transport First Aid"), "CTFA course should exist");
    ...
    <header className="sticky top-0 z-50 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6 md:py-4">
        <button type="button" onClick={() => goToPage("Home")} className="flex min-w-0 items-center gap-2 text-left sm:gap-3">
          <img onDoubleClick={openBackOffice} src={images.logoRound} alt="ACE MiDAS Training logo" className="h-10 w-10 shrink-0 rounded-full object-contain sm:h-12 sm:w-12" />
          <div className="min-w-0">
            <p className="truncate text-base font-bold leading-tight text-slate-950 sm:text-lg">ACE MiDAS Training</p>
            <p className="hidden text-xs text-slate-500 sm:block">Training â€¢ Compliance â€¢ Passenger Transport</p>
          </div>
        </button>
        ...
        <button type="button" onClick={() => setMenuOpen((current) => !current)} className="rounded-xl border border-slate-200 px-3 py-2 text-xl font-black text-slate-900 md:hidden" aria-label="Open menu">â˜°</button>
      </div>
...
function HomePage({ setPage, mediaSettings }) {
  const cards = [
    ...
    { title: "First Aid & CTFA", text: "First Aid at Work and Childrenâ€™s Transport First Aid with practical scenarios.", slot: "first_aid_at_work" }
  ];
  return (
    <main className="overflow-hidden">
      <section className="relative overflow-hidden bg-slate-950 text-white md:min-h-[88vh]">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-4 py-14 sm:px-6 sm:py-16 lg:grid-cols-[1.05fr_0.95fr] lg:py-28">
          <div>
            <div className="inline-flex items-center gap-3 rounded-full border border-emerald-400/30 bg-emerald-400/10 px-4 py-2 text-sm font-medium text-emerald-200"><span className="h-2 w-2 rounded-full bg-emerald-400" />MiDAS â€¢ PATS â€¢ First Aid â€¢ Compliance Hub</div>
            ...
            <p className="mt-6 max-w-2xl text-base leading-7 text-slate-300 sm:text-lg md:text-xl md:leading-8">Practical transport-focused training for schools, councils and operators â€” supported by a digital compliance hub for journey reporting, medication logs, attendance, wheelchair checks and incident evidence.</p>
...
function SubscribePage({ siteSettings }) {
  ...
  async function proceedToPayment() {
    ...
    setIsLoading(true);
    try {
      const response = await fetch("/api/create-subscribe-checkout-session", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ productType: "onboarding" }) });
      const data = await response.json();
      if (!response.ok || !data.url) throw new Error(data.error || "Unable to create Stripe checkout.");
      window.location.href = data.url;
    } catch (checkoutError) {
  ...
  function Section({ title, items }) {
    return <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-6"><h2 className="text-2xl font-black">{title}</h2><div className="mt-5 grid gap-3">{items.map((item) => <p key={item} className="rounded-2xl bg-slate-50 p-4 text-sm font-semibold text-slate-700">âœ” {item}</p>)}</div></section>;
  }
  return <main className="min-h-screen bg-slate-50 text-slate-950"><section className="bg-slate-950 px-4 py-14 text-white sm:px-6 sm:py-20"><div className="mx-auto max-w-5xl"><p className="font-semibold text-emerald-300">Subscribe for Access</p><h1 className="mt-4 text-2xl font-black sm:text-3xl tracking-tight sm:text-4xl md:text-6xl">Compliance & Tracking System â€“ Â£495 Setup Access</h1><p className="mt-6 max-w-4xl text-lg leading-8 text-slate-300">Give your transport operation the tools, structure and confidence to meet compliance standards and operate with complete peace of mind.</p><div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center"><button type="button" onClick={proceedToPayment} disabled={isLoading} className="w-full rounded-2xl bg-emerald-400 px-6 py-4 text-base font-black sm:w-auto sm:px-8 sm:text-lg text-slate-950 shadow-xl disabled:opacity-60">{isLoading ? "Opening secure checkout..." : "Secure Access & Setup â€“ Â£495"}</button><p className="text-sm font-semibold text-slate-300">Secure Stripe checkout. No hidden fees.</p></div>{error ? <p className="mt-5 rounded-xl bg-red-100 p-4 text-sm font-semibold text-red-800">{error}</p> : null}</div></section><div className="mx-auto grid max-w-7xl gap-6 px-6 py-16 lg:grid-cols-2"><Section title="What you get" items={valuePoints} /><Section title="Why this matters" items={reasons} /><Section title="Bespoke service" items={bespoke} /><Section title="Support and aftercare" items={support} /></div><section className="px-6 pb-20"><div className="mx-auto max-w-5xl rounded-3xl bg-emerald-500 p-8 text-center text-slate-950"><p className="text-2xl font-black md:text-4xl">Everything you need to run a compliant, accountable and professional transport operation â€” all in one place.</p><button type="button" onClick={proceedToPayment} disabled={isLoading} className="mt-8 rounded-2xl bg-slate-950 px-8 py-4 text-lg font-black text-white disabled:opacity-60">{isLoading ? "Opening secure checkout..." : "Proceed to Payment (Â£495)"}</button><p className="mt-3 text-sm font-semibold">Secure Stripe checkout. No hidden fees.</p></div></section></main>;
}
...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  const [code, setCode] = useState("");
  ...
  const [tcRecordForm, setTcRecordForm] = useState({ organisation_id: "", member_id: "", course_id: "", date_completed: "" });
  ...
  const tabs = ["Dashboard", "Blogs", "Reviews", "Onboarding", "Members", "Training Compliance", "Export Centre", "Depot Tokens", "Activity", "Media Manager", "Settings"];
  ...
  useEffect(() => {
    if (!unlocked || (activeTab !== "Training Compliance" && activeTab !== "Export Centre")) return;
  ...
  async function saveTrainingRecord(e) {
    ...
    try {
      ...
      setTcRecordForm({ organisation_id: "", member_id: "", course_id: "", date_completed: "" });
    ...
    <main className="min-h-screen overflow-x-hidden bg-slate-950 px-3 py-6 text-white sm:px-6 sm:py-10">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-4 rounded-2xl border border-white/10 bg-white/5 p-4">
          ...
          {debugOpen ? (
            <div className="mt-3 grid gap-2 text-xs text-slate-200">
              ...
              <p>Last saved setting timestamp: {debugInfo.lastSavedSettingTimestamp || "Not saved yet"}</p>
        ...
        <div className="mt-6 grid gap-4 lg:mt-8 lg:grid-cols-[240px_1fr] lg:gap-6">
          ...
          <section className="min-w-0 rounded-2xl bg-white p-4 text-slate-950 shadow-sm sm:p-6">
            {activeTab === "Dashboard" ? (
              <div>
              ...
              <div>
                ...
                <div className="mt-8 grid gap-6">
                  <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                    ...
                    <form onSubmit={saveTrainingOrganisation} className="mt-4 grid gap-4 md:grid-cols-5">
                      <input name="name" value={tcOrgForm.name} onChange={updateTcOrgForm} className="rounded-xl border border-slate-200 p-3 md:col-span-2" placeholder="Organisation name" required />
                      <input name="contact_name" value={tcOrgForm.contact_name} onChange={updateTcOrgForm} className="rounded-xl border border-slate-200 p-3" placeholder="Contact name" />
                      <input type="email" name="contact_email" value={tcOrgForm.contact_email} onChange={updateTcOrgForm} className="rounded-xl border border-slate-200 p-3" placeholder="Contact email" />
                      <input name="phone" value={tcOrgForm.phone} onChange={updateTcOrgForm} className="rounded-xl border border-slate-200 p-3" placeholder="Phone" />
                      <button type="submit" className="rounded-xl bg-slate-950 p-3 font-black text-white md:col-span-5">Add Organisation</button>
                    </form>
                    <div className="mt-5 grid gap-3">
                      {tcOrganisations.map((org) => <div key={org.id} className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 lg:grid-cols-[1.5fr_1fr_1fr_1fr_auto_auto]">
                        <input value={org.name || ""} onChange={(e) => updateTrainingOrganisation(org.id, "name", e.target.value)} className="rounded-lg border border-slate-200 p-2" />
                        <input value={org.contact_name || ""} onChange={(e) => updateTrainingOrganisation(org.id, "contact_name", e.target.value)} className="rounded-lg border border-slate-200 p-2" placeholder="Contact" />
                        <input value={org.contact_email || ""} onChange={(e) => updateTrainingOrganisation(org.id, "contact_email", e.target.value)} className="rounded-lg border border-slate-200 p-2" placeholder="Email" />
                        <input value={org.phone || ""} onChange={(e) => updateTrainingOrganisation(org.id, "phone", e.target.value)} className="rounded-lg border border-slate-200 p-2" placeholder="Phone" />
                  ...
                  <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                    ...
                    <form onSubmit={saveTrainingMember} className="mt-4 grid gap-4 md:grid-cols-5">
                      <select name="organisation_id" value={tcMemberForm.organisation_id} onChange={updateTcMemberForm} className="rounded-xl border border-slate-200 p-3" required><option value="">Organisation</option>{tcOrganisations.map((org) => <option key={org.id} value={org.id}>{org.name}</option>)}</select>
                      <input name="full_name" value={tcMemberForm.full_name} onChange={updateTcMemberForm} className="rounded-xl border border-slate-200 p-3" placeholder="Full name" required />
                      <input type="email" name="email" value={tcMemberForm.email} onChange={updateTcMemberForm} className="rounded-xl border border-slate-200 p-3" placeholder="Email" required />
                      <input name="role" value={tcMemberForm.role} onChange={updateTcMemberForm} className="rounded-xl border border-slate-200 p-3" placeholder="Role" required />
                      <button type="submit" className="rounded-xl bg-slate-950 p-3 font-black text-white">Add Staff</button>
                    </form>
                    <div className="mt-5 grid gap-3">
                      {tcMembers.map((member) => <div key={member.id} className="grid gap-3 rounded-xl border border-slate-200 bg-white p-4 lg:grid-cols-[1.2fr_1fr_1fr_1fr_auto_auto]">
                        <select value={member.organisation_id || ""} onChange={(e) => updateTrainingMember(member.id, "organisation_id", e.target.value)} className="rounded-lg border border-slate-200 p-2"><option value="">Organisation</option>{tcOrganisations.map((org) => <option key={org.id} value={org.id}>{org.name}</option>)}</select>
                        <input value={member.full_name || ""} onChange={(e) => updateTrainingMember(member.id, "full_name", e.target.value)} className="rounded-lg border border-slate-200 p-2" placeholder="Full name" />
                        <input value={member.email || ""} onChange={(e) => updateTrainingMember(member.id, "email", e.target.value)} className="rounded-lg border border-slate-200 p-2" placeholder="Email" />
                        <input value={member.role || ""} onChange={(e) => updateTrainingMember(member.id, "role", e.target.value)} className="rounded-lg border border-slate-200 p-2" placeholder="Role" />
                  ...
                  <section className="rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                    ...
                    <form onSubmit={saveTrainingRecord} className="mt-4 grid gap-4 md:grid-cols-5">
                      <select name="organisation_id" value={tcRecordForm.organisation_id} onChange={updateTcRecordForm} className="rounded-xl border border-slate-200 p-3" required><option value="">Organisation</option>{tcOrganisations.map((org) => <option key={org.id} value={org.id}>{org.name}</option>)}</select>
                      <select name="member_id" value={tcRecordForm.member_id} onChange={updateTcRecordForm} className="rounded-xl border border-slate-200 p-3" required><option value="">Staff member</option>{tcMembers.filter((member) => !tcRecordForm.organisation_id || member.organisation_id === tcRecordForm.organisation_id).map((member) => <option key={member.id} value={member.id}>{member.full_name}</option>)}</select>
                      <select name="course_id" value={tcRecordForm.course_id} onChange={updateTcRecordForm} className="rounded-xl border border-slate-200 p-3" required><option value="">Course</option>{tcCourses.map((course) => <option key={course.id} value={course.id}>{course.name}</option>)}</select>
                      <input type="date" name="date_completed" value={tcRecordForm.date_completed} onChange={updateTcRecordForm} className="rounded-xl border border-slate-200 p-3" required />
                      <button type="submit" className="rounded-xl bg-slate-950 p-3 font-black text-white">Add Record</button>
                    </form>
                    {tcRecordForm.date_completed && tcRecordForm.course_id ? <p className="mt-3 rounded-xl bg-white p-3 text-sm font-semibold text-slate-700">Expiry preview: {addMonthsToDate(tcRecordForm.date_completed, tcCourseMap[tcRecordForm.course_id]?.validity_months)} ({getTrainingStatus(addMonthsToDate(tcRecordForm.date_completed, tcCourseMap[tcRecordForm.course_id]?.validity_months))})</p> : null}
                      ...
                      <table className="min-w-[900px] w-full text-left text-sm">
                        ...
                        <tbody>
                          {filteredTrainingRecords.map((record) => {
                            ...
                            return <tr key={record.id} className="border-t border-slate-200"><td className="p-3">{record.organisation?.name || "Unknown"}</td><td className="p-3 font-bold">{record.staffMember?.full_name || "Unknown"}</td><td className="p-3">{record.staffMember?.email || ""}</td><td className="p-3">{record.staffMember?.role || ""}</td><td className="p-3">{record.course?.name || "Unknown"}</td><td className="p-3">{record.date_completed || ""}</td><td className="p-3">{record.expiry_date || ""}</td><td className="p-3"><span className={`rounded-full px-3 py-1 text-xs font-black ${statusClass}`}>{record.status}</span></td><td className="p-3"><button type="button" onClick={() => deleteTrainingRecord(record)} className="rounded-lg bg-red-600 px-3 py-2 text-xs font-black text-white">Delete</button></td></tr>;
                          })}
                        </tbody>
                      </table>
                    </div>
...
export default function App() {
  ...
  useEffect(() => {
    async function loadSupabaseContent() {
      ...
      if (!reviewError && reviewData && reviewData.length > 0) setReviews(reviewData.map((review) => ({ id: review.id, rating: review.rating || "â˜…â˜…â˜…â˜…â˜…", name: review.name || "Reviewer", org: review.organisation || "Organisation", text: review.content || "", status: review.status || "Draft" })));
    }
    loadSupabaseContent();
  }, []);
