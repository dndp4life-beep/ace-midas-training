...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  const [aiRules, setAiRules] = useState({
    ava: { approvalRequired: false, allowedActions: "Monitor compliance dashboard cards, detect expiry urgency, flag refresher risk, trigger safe reminder workflows", forbiddenActions: "Delete records, change expiry dates, mark training as complete", escalationTriggers: "Expired training, missing certificates, failed reminders" },
            ...
            {activeTab === "AI Operations" ? (
              <div>
                ...
                <section className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-5 shadow-sm">
                  ...
                  <p className="mt-3 text-sm leading-relaxed text-slate-700">Today&apos;s summary: {messagesReceivedToday} inbound message(s) received, {messagesRoutedToday} routed, {spamFilteredToday} filtered as spam/irrelevant and {theoApprovalsCreatedToday} Theo approval(s) created. Ava found {expiringWithinThirtyDays.length} record(s) expiring within 30 days. Mia sent {remindersSentToday} refresher reminder(s){miaWorkflowStats ? `, with ${miaWorkflowStats.skippedDuplicates || 0} duplicate(s) skipped and ${miaWorkflowStats.failedSends || 0} failed send(s) in the latest run` : ""}. Mia/Ellis classified {repliesClassifiedToday} customer reply/replies. Theo handled {theoBookingEnquiriesHandledToday} safe booking enquiry/enquiries, provided {theoEstimatesProvidedToday} estimate(s), referred {theoTrainingPageReferralsToday} customer(s) to the training page and sent {theoResponsesSentToday} booking response(s). Rory added or updated {prospectsAddedToday} prospect(s). Ellis filtered {emailsFilteredToday} email(s). Nia created {contentDraftsToday} content draft(s). Theo has {bookingApprovalsNeeded} booking approval item(s).</p>
