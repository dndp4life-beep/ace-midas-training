...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  const [aiRules, setAiRules] = useState({
    ...
    theo: { approvalRequired: false, allowedActions: "Answer safe booking process questions, qualify booking leads, explain group or onsite training in general terms, gather organisation/contact/course/attendee/location/timeframe/refresher/expiry details, refer customers to the training page, explain configured website discounts, and calculate indicative estimates only from configured pricing rules", forbiddenActions: "Confirm training dates, promise availability, confirm bookings, change bookings, cancel bookings, agree custom prices, confirm final quotes, offer non-standard discounts, send/request custom Stripe links", escalationTriggers: "Exact dates, confirmed availability, booking confirmations, booking changes, cancellations, pricing exceptions, custom discounts, custom prices, final quote/payment amount, payment links" },
  ...
  const aiAgentCards = [
    ...
    { key: "theo", role: "Training Bookings & Sales Coordinator", status: "Sales support with approval guardrails", actionsToday: agentLogsTodayFor("theo").length, pendingTasks: onboarding.filter((item) => (item.status || "New") !== "Completed").length + theoReplyApprovals.length, approvalsRequired: onboarding.length + theoReplyApprovals.length, lastActivity: agentLogsFor("theo")[0]?.created_at ? formatDisplayDateTime(agentLogsFor("theo")[0].created_at) : "Awaiting booking enquiry" },
