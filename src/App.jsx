...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  function handleReportAction(report) {
    if (report.action === "pdf") {
      showMessage("success", "PDF report generation coming next.");
      return;
    }
                  ...
                  <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    {premiumReportCards.map((report) => (
                      <article key={report.title} className="flex min-h-[220px] flex-col justify-between rounded-2xl border border-white/10 bg-white p-5 text-slate-950 shadow-sm transition-all duration-200 hover:-translate-y-1 hover:shadow-xl">
                        ...
                        <button type="button" onClick={() => handleReportAction(report)} className="mt-6 rounded-xl bg-slate-950 px-4 py-3 text-sm font-black text-white">
                          {report.action === "pdf" ? "Generate PDF" : report.action === "stub" ? "View Status" : "Generate / Export"}
                        </button>
                      </article>
                    ))}
                  </div>
