                      ...
                      {filteredOpportunities.length ? filteredOpportunities.map((opportunity) => <article key={opportunity.id} className={`rounded-2xl border p-4 ${opportunity.is_hot ? "border-red-200 bg-red-50" : "border-slate-200 bg-slate-50"}`}>
                        <div className="grid gap-4 lg:grid-cols-[1fr_auto] lg:items-start">
                          ...
                          <div className="grid gap-2 sm:grid-cols-2 lg:w-[360px]">
                            ...
                            <input type="number" min="0" step="0.01" defaultValue={Number(opportunity.estimated_value || 0)} onBlur={(event) => Number(event.target.value || 0) !== Number(opportunity.estimated_value || 0) && updateOpportunity(opportunity, { estimated_value: event.target.value }, "Estimated opportunity value updated.")} disabled={opportunityBusy === opportunity.id} className="rounded-lg border border-slate-300 bg-white p-2 text-xs font-black disabled:opacity-60" aria-label={`Estimated value for ${safeText(opportunity.organisation_name, "opportunity")}`} />
