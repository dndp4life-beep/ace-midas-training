...
function BackOfficePage({ setPage, posts, setPosts, reviews, setReviews, siteSettings, setSiteSettings, mediaSettings, setMediaSettings }) {
  ...
  async function saveTrainingOrganisation(e) {
    ...
    }).select("id, name, contact_name, contact_email, phone, created_at").single();
    if (error) {
      console.error("Organisation save error:", error);
      showMessage("error", error.message || "Could not save changes. Please try again.");
      return;
    }
    const { data: refreshedOrganisations, error: refreshError } = await supabase.from("organisations").select("id, name, contact_name, contact_email, phone, created_at").order("name", { ascending: true });
    if (refreshError) console.error("Organisation refresh error:", refreshError);
    setTcOrganisations(refreshedOrganisations || ((current) => [...current, data].sort((a, b) => String(a.name).localeCompare(String(b.name)))));
    setTcOrgForm({ name: "", contact_name: "", contact_email: "", phone: "" });
    setActivity((current) => [`Organisation added: ${data.name}`, ...current]);
  ...
  async function saveTrainingOrganisationRow(org) {
    ...
    if (error) console.error("Organisation update error:", error);
    showMessage(error ? "error" : "success", error ? (error.message || "Could not save changes. Please try again.") : "Organisation saved successfully");
  }
...
export default function App() {
  ...
  useEffect(() => {
    async function loadSiteSettings() {
      ...
      if (data?.setting_value) setSiteSettings((current) => ({ ...current, ...data.setting_value }));
    }
    loadSiteSettings();
  }, []);
