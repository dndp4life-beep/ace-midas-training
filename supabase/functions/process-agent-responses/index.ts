...
Deno.serve(async (req) => {
  ...
  for (const item of pending || []) {
    ...
    try {
      ...
      const sent = await sendEmail(resendKey, { from: sender, to: [item.from_email], subject, html: html(generated.body) });
      ...
      await logAgent(supabase, item.assigned_agent, `${item.assigned_agent}_dynamic_response_sent`, `${agents[item.assigned_agent]?.name || "Agent"} sent a dynamic Manus response using approved knowledge base context.`, "sent", { ...metadata, dynamic_response_used: true, manus_task_id: generated.task_id, email_sent: true, knowledge_matches: knowledge.matches });
      await logAudit(supabase, item.assigned_agent, "dynamic_agent_response_sent", "Dynamic agent response sent through Manus, Resend and approved knowledge base context.", "sent", { ...metadata, dynamic_response_used: true, manus_task_id: generated.task_id, email_sent: true, knowledge_matches: knowledge.matches });
      summary.sent += 1;
