...
async function triggerAgentProcessor(supabaseUrl: string, functionInvokeKey: string, supabase: ReturnType<typeof createClient>, metadata: Record<string, unknown>) {
  try {
    ...
    await logAgent(supabase, "ellis", isTimeout ? "agent_processor_trigger_timed_out" : "agent_processor_trigger_failed", isTimeout ? "Ellis queued the response processor, but the trigger took too long to confirm." : "Ellis could not trigger the agent response processor.", isTimeout ? "queued_unconfirmed" : "failed", {
      ...metadata,
      processor_error: isTimeout ? "Processor trigger timed out after 2.5 seconds so the contact form could complete." : error instanceof Error ? error.message : "Unknown processor trigger error.",
    });
  }
}
...
${args.estimate ? `- Indicative estimate: ${args.estimate.attendee_count} attendee(s), ${args.estimate.course_title}, listed price ${args.estimate.listed_price}, approximate subtotal GBP ${args.estimate.estimated_subtotal}. Say it is indicative and excludes travel/custom requirements.` : ""}
...
async function sendTheo(resendApiKey: string, sender: string, to: string, name: string, currentRoute: any, currentEstimate: ReturnType<typeof estimate>, dynamicText: string) {
  ...
  const est = currentEstimate ? `<p>For ${esc(currentEstimate.attendee_count)} attendee(s) on ${esc(currentEstimate.course_title)}, the indicative course subtotal is approximately GBP ${esc(currentEstimate.estimated_subtotal)} before travel fees or custom requirements.</p>` : "";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const anonInvokeKey = Deno.env.get("SUPABASE_ANON_KEY") || "";
    const functionInvokeKey = anonInvokeKey || serviceRoleKey;
    const functionInvokeKeySource = anonInvokeKey ? "SUPABASE_ANON_KEY" : "SUPABASE_SERVICE_ROLE_KEY_FALLBACK";
