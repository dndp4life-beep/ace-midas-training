...
const THEO_COMPLIANCE_PAGE_URL = "https://www.ace-midas-training.co.uk/compliance";
const THEO_BOOKING_KNOWLEDGE = {
  identity: {
    name: "Theo",
  ...
  training_page_url: THEO_TRAINING_PAGE_URL,
  compliance_page_url: THEO_COMPLIANCE_PAGE_URL,
...
const THEO_GROUP_DISCOUNT_RULES = [
...
function classifyTheoBookingIntent(text: string) {
  ...
  if (hasAny(["price", "pricing", "cost", "quote", "how much", "estimate"])) {
    return { classification: "pricing enquiry", assigned_agent: "theo", status: "auto_reply", action_taken: "Theo can explain pricing and calculate estimates only from configured pricing rules", approval_required: false, decision_reason: "Standard pricing enquiries can be answered safely when estimates are labelled and based on configured rules." };
  }
  if (hasAny(["book", "booking", "attendees", "delegates", "drivers", "refresher booking", "refresher training", "full training", "multiple staff"])) {
...
function calculateTheoEstimate(message: string) {
  ...
  const discountRule = THEO_GROUP_DISCOUNT_RULES.find((rule) => attendees >= rule.minimum_attendees);
  ...
  const discountText = saving > 0 ? `${discountRule.label} applied, saving approximately GBP ${saving}.` : "No configured group discount is triggered at this attendee number.";
  return {
...
function extractTheoLeadQualification(message: string, inbound: Record<string, unknown> = {}) {
  const text = String(message || "");
  const lower = text.toLowerCase();
  const course = THEO_BOOKING_KNOWLEDGE.course_categories.find((name) => lower.includes(name.toLowerCase())) || "";
  ...
  return {
    ...
    contact_name: String(inbound.name || "").trim(),
    contact_email: String(inbound.email || "").trim(),
...
async function sendTheoSafeResponse(resendApiKey: string, sender: string, to: string, name: string, route: ReturnType<typeof classifyInboundMessage>, estimate: ReturnType<typeof calculateTheoEstimate>) {
  ...
  let nextStep = "Please reply with the course required and number of attendees.";
  let includeTrainingPage = true;
  let includeSafetyNote = false;

  if (classification === "group booking enquiry") {
    answer = "Group bookings can be more cost-effective when several staff need training or refreshers at the same time.";
    nextStep = "Please send the course required and number of attendees.";
  } else if (classification === "onsite training enquiry") {
    answer = "Onsite training may be possible depending on the course, attendee numbers, facilities and location.";
    nextStep = "Please send your site location and the course required.";
    ...
    nextStep = "Please send your preferred timeframe and location.";
    includeSafetyNote = true;
  } else if (classification === "pricing enquiry") {
    answer = estimate ? "I can give an indicative estimate from the configured pricing rules." : "Pricing varies by course and group size.";
    nextStep = estimate ? "For final pricing or booking arrangements, the team will confirm before anything is agreed." : "Please send the course required and number of attendees.";
  } else if (classification === "booking process question") {
    answer = "To start a booking request, send the key training details and the team will review the best route.";
    nextStep = "Please send the course required and number of attendees.";
  } else if (classification === "booking information request") {
    answer = "That sounds like a booking enquiry, so the next step is to confirm the basic training requirement.";
    nextStep = "Please send the course required and number of attendees.";
  }

  const estimateHtml = estimate ? `
      <p>Based on the configured pricing rules, an indicative estimate for ${escapeHtml(estimate.attendee_count)} attendee(s) on ${escapeHtml(estimate.course_title)} is GBP ${escapeHtml(estimate.estimated_subtotal)} before any travel fee or custom requirements.</p>
      <p>${escapeHtml(estimate.discount_text)}</p>
    ` : "";
  const trainingPageHtml = includeTrainingPage ? `<p>Current training options and visible group booking discounts are available here: <a href="${THEO_TRAINING_PAGE_URL}">${THEO_TRAINING_PAGE_URL}</a></p>` : "";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const qualification = route.assigned_agent === "theo" ? extractTheoLeadQualification(message, { name, email, organisation }) : null;
    const estimate = route.assigned_agent === "theo" && !route.approval_required ? calculateTheoEstimate(message) : null;
    const responseType = route.assigned_agent === "theo" ? (route.approval_required ? "approval_required" : estimate ? "estimate_response" : route.classification === "pricing enquiry" ? "request_more_information" : "safe_auto_reply") : "routed";
    const metadata = {
    ...
    if (route.assigned_agent === "theo" && route.approval_required) {
      ...
      let theoTrace = createTheoDecisionTrace(inboundMessage?.id || "", route, { resend_send_attempted: true, pricing_estimate_calculated: Boolean(estimate), training_page_referred: true, pricing_estimate: estimate, lead_qualification: qualification, response_type: responseType, booking_knowledge: THEO_BOOKING_KNOWLEDGE });
