...
async function triggerAgentProcessor(supabaseUrl: string, serviceRoleKey: string, supabase: ReturnType<typeof createClient>, metadata: Record<string, unknown>) {
  try {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (currentRoute.assigned_agent === "theo" && currentRoute.approval_required) {
      ...
      const processorPromise = triggerAgentProcessor(supabaseUrl, serviceRoleKey, supabase, metadata);
      const edgeRuntime = (globalThis as unknown as { EdgeRuntime?: { waitUntil?: (promise: Promise<unknown>) => void } }).EdgeRuntime;
      if (edgeRuntime?.waitUntil) edgeRuntime.waitUntil(processorPromise);
      else processorPromise.catch((error) => console.error("Agent processor background trigger error:", error));
    } else {
