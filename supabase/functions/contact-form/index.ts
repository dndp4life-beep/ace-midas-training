...
async function sendTheoSafeResponse(resendApiKey: string, sender: string, to: string, name: string, route: ReturnType<typeof classifyInboundMessage>) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (route.assigned_agent === "theo" && route.approval_required) {
      ...
      const theoResponse = await sendTheoSafeResponse(resendApiKey, sender, email, name, route);
      if (theoResponse.ok) {
        await logAgentActivity(supabase, "theo", "theo_auto_response_sent", "Theo sent a safe booking information response.", "sent", metadata);
        await logAuditActivity(supabase, "theo", "theo_auto_response_sent", "Theo sent a safe booking information response.", "sent", metadata);
      } else {
        const theoError = await theoResponse.text();
        console.error("Theo safe response error:", { status: theoResponse.status, body: theoError });
        await logAgentActivity(supabase, "theo", "theo_auto_response_failed", "Theo could not send the safe booking information response.", "failed", { ...metadata, error: theoError });
      }
