...
const forbiddenCustomerPhrases = [
  ...
  "system decision",
  "workflow status",
  "this message does not confirm",
  "does not confirm dates",
];

const dynamicResponseRules = `
You write customer-facing emails for ACE MiDAS Training.

Business context:
- ACE MiDAS Training is a training compliance, refresher tracking, certificate record, reminder automation and AI-assisted business workflow platform.
- It provides MiDAS, PATS and First Aid training support, plus compliance app/service information including ACE Compliance Hub / SafeJourney-style compliance support, medication tracking, journey tracking, compliance records, certificates, reminders, reports and operational setup support.
- Do not describe ACE as a transport company operations/fleet/incident system.

Customer email rules:
- Answer the customer's actual question first.
- Use plain English, warm professional tone, no hype.
- Give one useful next step.
- Never expose internal routing, classification, approval rules, workflow status or system decisions.
- Do not say "routed", "classified", "assigned agent", "approval_required", "workflow", "internal rules", or "this message does not confirm".
...
Agent rules:
- Mia answers general service enquiries, training information, refresher training, PATS, MiDAS, First Aid, compliance app/service enquiries, monthly service fee enquiries in general terms and website referrals.
- Theo answers booking process, group booking, onsite training, safe pricing estimates from approved rules, and booking preparation questions.
...
- Ellis classifies/routes only and must not expose routing details.
- Rory researches prospects only and must not contact prospects.

Pricing rules:
- Theo may only calculate estimates from approved listed prices supplied in the prompt.
- Monthly/service fee questions can be answered in general terms only unless an approved price is supplied.
- Final pricing, custom pricing, discounts outside visible website rules, and payment links require admin handling.

Output:
- Return only the email body as plain text.
- Include a greeting and the correct signature.
- Keep it concise: 4 to 8 short paragraphs maximum.
`;
...
function stripCustomerInternalLanguage(text: string) {
  return text
    .split(/\r?\n/)
    .filter((line) => {
      const lower = line.toLowerCase();
      return !forbiddenCustomerPhrases.some((phrase) => lower.includes(phrase));
    })
    .join("\n")
    .replace(/\n{3,}/g, "\n\n")
    .trim();
...
function ensureAgentSignature(text: string, agentKey: string) {
  const identity = agentIdentities[agentKey] || agentIdentities.mia;
  if (text.toLowerCase().includes(identity.name.toLowerCase()) && text.includes("ACE MiDAS Training")) return text;
  return `${text.trim()}\n\nKind regards,\n${identity.name}\n${identity.title}\nACE MiDAS Training`;
}
...
function buildDynamicResponseInput(args: {
  agentKey: string;
  name: string;
  organisation: string;
  enquiryType: string;
  message: string;
  route: any;
  estimate: ReturnType<typeof calculateTheoEstimate>;
}) {
  const identity = agentIdentities[args.agentKey] || agentIdentities.mia;
  return `
Agent: ${identity.name}
Agent title: ${identity.title}
Customer name: ${args.name || "there"}
Organisation: ${args.organisation || "Not provided"}
Enquiry type: ${args.enquiryType || "Website enquiry"}
Classification: ${args.route?.classification || "not set"}
Assigned agent: ${args.route?.assigned_agent || args.agentKey}

Customer message:
${args.message}

Approved pricing/context:
- Training page: ${trainingPageUrl}
- Compliance page: ${compliancePageUrl}
- Approved course prices: ${pricingRules.map((rule) => `${rule.title}: ${rule.price}`).join("; ")}
- Website group discount information may be mentioned generally only when relevant; do not invent a discount amount unless supplied.
- Compliance app/service enquiries: explain ACE Compliance Hub / compliance app support in general terms as a system/service for compliance records, training/certificate oversight, reminders, medication/journey tracking where relevant, reporting and setup support. If asked about monthly fees, explain that the monthly/service cost depends on the package, tools and organisation requirements unless an approved figure is supplied, and invite them to share what they need or view the compliance page.
${args.estimate ? `- Indicative Theo estimate: ${args.estimate.attendee_count} attendee(s), ${args.estimate.course_title}, listed price ${args.estimate.listed_price}, approximate course subtotal GBP ${args.estimate.estimated_subtotal}. Mention it is indicative and excludes travel/custom requirements.` : ""}

Write the customer email now.`;
}

async function generateDynamicAgentResponse(args: {
  apiKey: string;
  agentKey: string;
  name: string;
  organisation: string;
  ...
  try {
    const response = await fetch("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify({
        model: openAiModel,
        instructions: dynamicResponseRules,
        input: buildDynamicResponseInput(args),
    ...
    if (!response.ok) {
      const errorMessage = data?.error?.message || text || "OpenAI response generation failed.";
      console.error("OpenAI response error:", { status: response.status, error: errorMessage });
      return { used: false, provider: "openai", model: openAiModel, text: "", error: errorMessage };
    }

    const raw = extractOpenAiText(data);
    const cleaned = ensureAgentSignature(stripCustomerInternalLanguage(raw), args.agentKey);
    if (!cleaned) return { used: false, provider: "openai", model: openAiModel, text: "", error: "OpenAI returned an empty response." };

    return { used: true, provider: "openai", model: openAiModel, text: cleaned, error: "" };
  } catch (error) {
    const message = error instanceof Error ? error.message : "OpenAI response generation failed.";
    console.error("OpenAI response exception:", error);
    return { used: false, provider: "openai", model: openAiModel, text: "", error: message };
  }
}
...
async function sendMiaSafeResponse(resendApiKey: string, sender: string, to: string, name: string, enquiryType: string, dynamicText = "") {
  const html = dynamicText
    ? `<div style="font-family: Arial, sans-serif; line-height: 1.6; color: #0f172a;">${plainTextToHtml(dynamicText)}</div>`
    : `
      <div style="font-family: Arial, sans-serif; line-height: 1.6; color: #0f172a;">
        ...
        <p>PATS training is designed for passenger assistants who support children, young people or vulnerable passengers during transport. MiDAS training is designed for minibus drivers and focuses on safer, more confident minibus driving.</p>
        <p>ACE MiDAS Training provides PATS, MiDAS and First Aid training support for organisations that need staff to stay trained and up to date.</p>
        <p>If helpful, we can also explain which course route may be most suitable for your staff.</p>
...
async function sendTheoSafeResponse(resendApiKey: string, sender: string, to: string, name: string, route: any, estimate: ReturnType<typeof calculateTheoEstimate>, dynamicText = "") {
  if (dynamicText) {
    return sendEmail(resendApiKey, {
      from: sender,
      to: [to],
      subject: "Booking information from ACE MiDAS Training",
      html: `<div style="font-family: Arial, sans-serif; line-height: 1.6; color: #0f172a;">${plainTextToHtml(dynamicText)}</div>`,
    });
  }
  ...
  if (classification === "pricing enquiry") {
    answer = estimate ? "Based on the details provided, here is an indicative estimate using the current course pricing." : "Pricing depends on course type, attendee numbers and location.";
  }
  ...
  const estimateHtml = estimate ? `<p>For ${escapeHtml(estimate.attendee_count)} attendee(s) on ${escapeHtml(estimate.course_title)}, the indicative course total is approximately GBP ${escapeHtml(estimate.estimated_subtotal)} before any travel fee or additional requirements.</p>` : "";
  const safetyHtml = includeSafetyNote ? "<p>Please send your preferred timeframe and location, and the team can advise on the next suitable step.</p>" : "";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const dynamicResponse =
      openAiApiKey && ["mia", "theo"].includes(String(route.assigned_agent)) && !route.approval_required
        ? await generateDynamicAgentResponse({
            apiKey: openAiApiKey,
            agentKey: String(route.assigned_agent),
            name,
            organisation,
            enquiryType,
            message,
        ...
        : {
            used: false,
            provider: openAiApiKey ? "openai" : "",
            model: openAiApiKey ? openAiModel : "",
            text: "",
            error: openAiApiKey ? "" : "OPENAI_API_KEY is not configured for dynamic responses.",
          };
    ...
    const metadata = {
      ...
      dynamic_response_used: dynamicResponse.used,
      ai_provider: dynamicResponse.provider,
      ai_model: dynamicResponse.model,
      ai_error: dynamicResponse.error,
      email_sent: false,
    };
    ...
    if (route.assigned_agent === "theo" && route.approval_required) {
      ...
      const theoResponse = await sendTheoSafeResponse(resendApiKey, sender, email, name, route, estimate, dynamicResponse.text);
      if (theoResponse.ok) {
        await logAgentActivity(supabase, "theo", "theo_auto_response_sent", "Theo sent a safe booking information response.", "sent", { ...metadata, email_sent: true, dynamic_response_used: dynamicResponse.used });
        await logAuditActivity(supabase, "theo", "theo_auto_response_sent", "Theo sent a safe booking information response.", "sent", { ...metadata, email_sent: true, dynamic_response_used: dynamicResponse.used });
      } else {
      ...
      const miaResponse = await sendMiaSafeResponse(resendApiKey, sender, email, name, enquiryType, dynamicResponse.text);
      if (miaResponse.ok) {
        await logAgentActivity(supabase, "mia", "mia_contact_response_sent", "Mia sent a safe contact form acknowledgement.", "sent", { ...metadata, email_sent: true, dynamic_response_used: dynamicResponse.used });
        await logAuditActivity(supabase, "mia", "mia_contact_response_sent", "Mia sent a safe contact form acknowledgement.", "sent", { ...metadata, email_sent: true, dynamic_response_used: dynamicResponse.used });
      } else {
