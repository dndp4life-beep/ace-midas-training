...
async function sendTheoSafeResponse(resendApiKey: string, sender: string, to: string, name: string, route: ReturnType<typeof classifyInboundMessage>, estimate: ReturnType<typeof calculateTheoEstimate>) {
  ...
  let guidance = "To help the team advise you properly, please reply with the organisation name, contact details, course type, number of attendees, training location, preferred timeframe, whether this is refresher or full training, and any urgent expiry deadline.";

  if (classification === "group booking enquiry") {
    guidance = `${THEO_BOOKING_KNOWLEDGE.group_booking_explanation} Please send the course type, approximate number of delegates, training location, and preferred timeframe so the team can review the best next step.`;
  } else if (classification === "onsite training enquiry") {
    guidance = `${THEO_BOOKING_KNOWLEDGE.onsite_training_explanation} Please send your site location, course type, number of attendees and preferred timeframe.`;
